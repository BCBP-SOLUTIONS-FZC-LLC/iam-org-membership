...
2. Build the event in the relevant service method and pass it to `port.EventPublisher.Enqueue` inside the same `RunInTx` as the business write (EVT-10, CONS-1). The `RoutingPublisher` selects the topic (`iam.membership.events` vs `iam.tenant.events`) from `Envelope.Source`.

...
- **T-13** — `ownerless_since` set only by TM-12 (I-5 removing last active `tenant_owner`); cleared only by O-7 (operator reassignment). Backs `iam_org_membership_tenant_ownerless` alert.

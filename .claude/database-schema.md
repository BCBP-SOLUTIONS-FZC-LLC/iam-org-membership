...
CREATE TYPE tender_acl_level      AS ENUM ('view','edit','approve');  -- §16 A32(c)/rev 1.27: HLD-aligned; was read/write/admin
...
CREATE TYPE branding_level        AS ENUM ('none','logo');             -- §16 A19: HLD-trimmed to logo-only
```
...
15 tables total. Every soft-deletable table's uniqueness constraint is a **partial unique index** (`WHERE deleted_at IS NULL`, or `WHERE status='pending'` for `pending_invitations`) so a user can rejoin a tenant/department they previously left (the TM-11 / DM-3 / TAE-1 / PI-1 rejoin bug fixed rev 0.17/0.18). Config tables without `deleted_at` use ordinary full `UNIQUE`.

| Table | Key columns / notable constraints |
|---|---|
| `tenants` | Root: `id`, `slug` (immutable via `trg_tenant_slug_immutable`), `plan`, `feature_flags jsonb` (override delta only; §16 A18/T-9), `status`, `trial_ends_at`, `trial_reactivation_count` (CHECK 0..1, T-14), `subscription_started_at`, `cancelled_at` (T-11), `last_event_at` (EVT-14 high-water mark), `realm_id`, `realm_type` (§16 A22, drives partial-unique on realm_id), `keycloak_shard` (§16 A23, T-12; RP-owned projection), `mfa_freshness_seconds` (T-10, 60–900 default 300), `local_accounts_enabled`, `realm_sync_pending` (T-15), `default_locale`, `licensed_seats` (SEAT-1..SEAT-5), `ownerless_since` (T-13), `overage_since` (SEAT-5), `record_version`. `uq_tenants_slug` (full unique), `uq_tenants_realm_id_dedicated WHERE realm_type='dedicated'`. Partial indexes: `idx_tenants_ownerless`, `idx_tenants_realm_sync_pending`, `idx_tenants_seat_overage`. Checks: `chk_trial_ends_at_required`, `chk_subscription_started_required`, `chk_offboarded_soft_deleted` (PAID-1), `chk_cancelled_at_required` (T-11, two-directional). FK: `fk_tenants_plan → plans(code)`. |
| `plans` (§16 A19) | Global operator entitlement catalog. PK `code` (reuses `tenant_plan` enum). Columns: `display_name`, `trial_duration_days` (§16 A32(g); drives `trial_ends_at`), `workflow_template_limit`, `tender_limit`, `sso_enabled`, `custom_branding branding_level`, `feature_set jsonb`. Operator PATCH-only (O-6, PLAN-4); no create/delete. **Limits are NULLABLE with NULL = unlimited** (resolves the LLD §4.2 `-1` sentinel ambiguity per §19.3 recommendation). Seed: starter (5 workflows / 10 tenders), pro (50 / 100), enterprise (NULL / NULL = unlimited). |
| `departments` | Global operator catalog. `code` UNIQUE + **immutable** (D-2/D-10), `is_system` **immutable** (D-2), `is_active`, `chk_system_department_active` (`NOT (is_system AND NOT is_active)`) — a system department cannot be retired without an operator migration clearing `is_system` first (D-9/D-11). Never physically deleted (OP-3): `DELETE` returns 405 and a `BEFORE DELETE` trigger backstops. **5 system departments seeded**: Engineering, Design, Procurement, Finance, Legal (per LLD §4.2). |
| `tenant_departments` | Composite PK `(tenant_id, department_id)`; join activating a catalog dept for a tenant; `is_active`. |
...
| `group_dept_role_mappings` (renamed from `group_role_mappings`, §16 A25) | Keycloak group → dept-role level. `uq_group_dept_role_mapping = UNIQUE (tenant_id, keycloak_group_name)` (GDRM-1). |
| `group_tenant_role_mappings` (new §16 A25) | Keycloak group → tenant-level role (elevated only; `chk_gtrm_no_member` bars `member`, GTRM-6). Closes gap where `Tender-Admins` group couldn't resolve to `tenant_admin`. `uq_group_tenant_role_mapping` (GTRM-1). |
| `group_dept_mappings` | Keycloak group → department. `uq_group_dept_mapping = UNIQUE (tenant_id, keycloak_group_name, department_id)` (GDM-1). |
| `delegations` | Authoritative delegation grant. `delegator_membership_id`/`delegate_membership_id` composite FKs → `tenant_memberships` (§16 A16/DEL-9 — pins both parties to the same tenant). `scope` (all|department|tender), `scope_id`, `starts_at`, `ends_at`, `status`, `reason` (audit-only, §16 A32(f), DEL-10). Indexes: `idx_delegations_delegator (tenant_id, delegator_id) WHERE deleted_at IS NULL AND status='active'` (hot-path I-8 join); `idx_delegations_ends_at (ends_at) WHERE deleted_at IS NULL AND status='active' AND ends_at IS NOT NULL` (expiry job). |
| `tender_acl_entries` | Additive tender grants. `access_level ENUM (view|edit|approve)`. `tenant_membership_id` composite FK (§16 A16/TAE-8). `uq_tae_active_entry = UNIQUE (tenant_id, tender_id, user_id) WHERE deleted_at IS NULL` (TAE-1). `granted_by` NOT NULL, `reason` (nullable, 500 char cap), `expires_at` (nullable, future-only enforced at P-22). Active-grant SQL: `WHERE deleted_at IS NULL AND (expires_at IS NULL OR expires_at > now())` (TAE-3, used by I-12 service-to-service check). |
...
Every tenant-scoped table (all except `departments`, `plans`, `processed_events`) has:
```sql
...
- Forward-only, additive. `migrate.Runner{DSN}.Up(ctx)` at startup. `outbox.ApplySchema` runs **after** business migrations (MIG-2).
...
- **T-11** — `cancelled_at IS NOT NULL ⇔ status ∈ (cancelled, suspended, offboarded)` (two-directional CHECK). Set/cleared in lockstep with status by `TenantSubscriptionCancelled`/`TenantReactivated`.
...
- **CONC-1** — All 14 `record_version`-carrying tables use optimistic locking: `tenants`, `departments`, `tenant_departments`, `tenant_memberships`, `tenant_roles`, `dept_memberships`, `dept_role_labels`, `group_dept_role_mappings`, `group_tenant_role_mappings`, `group_dept_mappings`, `delegations`, `tender_acl_entries`, `pending_invitations`, `plans`. `processed_events` excluded.

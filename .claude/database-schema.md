...
| `group_dept_mappings` | Keycloak group → department. `uq_group_dept_mapping = UNIQUE (tenant_id, keycloak_group_name, department_id)` (GDM-1). `department_id` no longer has a DB FK (`fk_gdm_department` dropped, Phase 4) — checked in `group_mapping_service.go::ReplaceDept` against `port.DepartmentCatalogReader`. |

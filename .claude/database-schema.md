...
- **CONC-1** — All 9 `record_version`-carrying tables use optimistic locking: `tenants`, `tenant_departments`, `tenant_memberships`, `tenant_roles`, `dept_memberships`, `dept_role_labels`, `delegations`, `tender_acl_entries`, `pending_invitations`. (`departments`/`plans` moved to the Catalog Service in Phase 4; `group_dept_role_mappings`/`group_tenant_role_mappings`/`group_dept_mappings` moved to Group Mapping Service in Wave 2 — see the schema-ownership notes above.) `processed_events` excluded.

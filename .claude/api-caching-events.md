...
**EVT-15 future-time clamp (§16 A40, poison-pill guard):** If `event.time > now() + MAX_LIFECYCLE_EVENT_SKEW_SECONDS` (default 300 s), event is **rejected to DLQ** (not applied, `last_event_at` not advanced, **not** recorded in `processed_events`). `platform_dlq_messages_total`++ — any nonzero pages (a producer's clock is skewed). Mechanism: the handler returns `ErrPoisonPill`; `cmd/server/dlq.go`'s `routePoisonPillsToDLQ` `SendMessage`s the (already-decoded, `dataschema`-cleared) envelope straight to the queue's DLQ — resolved at startup from the queue's own `RedrivePolicy` — with `DLQReason=future_time_clamp`, then acks. platform-events treats every handler error as retryable, so without this the clamp would be re-evaluated on each redelivery. If the DLQ can't be resolved or the send fails, it falls back to SQS redrive after `maxReceiveCount`. Inbound consumers also carry `eventbus.GlueDecoder` (`events.WithConsumerCodec`), so Glue-encoded upstream envelopes (non-empty `dataschema`) are header-stripped before the handler runs.

**Consumed-event schema validation:** every inbound queue runs `cmd/server/inbound_schema.go`'s `validateConsumed` before `Handle` — the decoded payload is checked against the **embedded** consumed schema for `env.Type` (the same compiled `schemas/*.json` `ValidatingCodec` uses for produced events; no Glue call, works through a Glue outage). It validates this service's own contract (what `asyncapi.yaml` says it relies on), not the producer's Glue schema. Schemas are OPEN, so new producer fields pass. A violation increments `platform_dlq_messages_total{reason="schema_violation"}` and goes straight to the DLQ (`DLQReason=schema_violation`) via the same router as EVT-15, without reaching the projection or `processed_events`. An event type with no embedded schema passes through to `ackUnknown`. Pipeline order per queue (`inboundHandler`): DLQ routing → `platform_messages_*` counters → schema validation → `Handle`.

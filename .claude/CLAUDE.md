...
- **`internal/adapter/outbound/postgres/*_repository.go`** — implements all `core/port` repositories; all 9 `record_version`-carrying tables use `UPDATE ... WHERE id=$1 AND record_version=$2` optimistic locking (409 `optimistic_lock_conflict`, TRG-1 client never sets `record_version`).
...
- **`internal/adapter/outbound/{userprofile,workflow,realmprovisioner,catalogadmin,groupmappingclient}/http_client.go`** — five outbound service clients, identical construction pattern (`gincommon.PropagateHeaders`, configurable `*_TIMEOUT_MS` env var); `catalogadmin`/`groupmappingclient` are read-only (departments/plans; group→role/department mapping resolution) and errors are never fabricated into safe defaults — resilience is the caller's `CatalogService`/`GroupMappingService` cache + stale-if-error layer, not the client.
...
- [`database-schema.md`](database-schema.md) — 10 tables, enums, RLS/tenant/seat/migration/trigger invariants

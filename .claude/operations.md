...
| Metric | Type | Labels | Description |
|---|---|---|---|
| `iam_rls_violations_total` | Counter | `violation_type` | **Fixed 2026-09-21** — previously registered but never incremented. Closed by a 5th `cmd/server/exporters.go` goroutine, mirroring `iam-user-profile`'s `runRLSViolationExporter`/`MaintenanceSweeper.RLSViolationCounts` pattern: `pgadapter.GaugeRepository.RLSViolationCounts(ctx, window)` (BYPASSRLS `sysPool`, `GROUP BY violation_type` over the trailing 5-minute window) runs on the same `exporterInterval` ticker as the other 4, incrementing this counter by the per-type delta each tick. Counter is monotone by design — alerts use `rate()`/`increase()` over the same window, stable across pod restarts. Verified against real Postgres (`test/postgres/gauge_repository_test.go`'s `TestGaugeRepo_RLSViolationCounts_GroupsByTypeWithinWindow`) |
...
| `outbox-prune` | `0 3 * * *` | Batched delete at `OUTBOX_RETENTION_DAYS` (default 8) retention via `platform-events`' own `outbox.Runner.PrunePublished(ctx, retention, batchLimit)`** — `cmd/reconciler`'s job (`OutboxPrune`, `cmd/reconciler/jobs/outbox_prune.go`) builds a real (never-`Start()`ed, publisher-less) `*outbox.Runner`, wired onto `jobs.Context.OutboxRunner` by `cmd/reconciler/main.go`, purely to call this one method; a nil `OutboxRunner` (e.g. an un-updated test helper) makes the job no-op (`Succeeded=0`, no error) rather than fail. `port.ReconcilerStore.PruneOutbox` — the hand-rolled batched-SQL delete this migration replaced — is fully **removed 2026-09-21** (interface method, implementation, and all dedicated tests/fakes deleted, not left as reachable dead code). See the "Events / Outbox / Dedup Rule" notes in `.claude/architecture.md` |

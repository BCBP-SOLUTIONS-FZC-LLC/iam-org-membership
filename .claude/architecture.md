...
- `httptransport` (`internal/adapter/outbound/httpx`) — a shared, otelhttp-instrumented `http.Client` factory (`httpx.NewClient(timeout)`) meant to replace every outbound client's own manual `http.Client` construction + per-package `propagateTraceparent` helper. **Scaffolded but not yet adopted**: nothing outside its own test file imports it today — all five outbound clients (`workflow`, `realmprovisioner`, `catalogadmin`, `groupmappingclient`, `delegationcheck`) still build their `http.Client` by hand and inject `traceparent` via their own local `propagateTraceparent(ctx, req)` (raw `otel`/`propagation`, not `httpx`, not `gincommon.PropagateHeaders`). Declared in `.go-arch-lint.yml` as its own component (`anyVendorDeps: true`) so `adapters_outbound` may depend on it once the migration actually happens; don't assume from its presence in the tree that the migration is done.

...
| Rev | Date | Change |
|---|---|---|
| 2.8 | 2026-09-21 | **Real CI run against the pinned `platform-schemagov:0.4` image confirmed both schema-governance caveats rev 2.6 flagged as unconfirmed — fixed.** (1) Pass 5's open-schema guard: all 27 pre-existing `internal/adapter/outbound/eventbus/schemas/*.json` files had `additionalProperties: false`; changed to `additionalProperties: true` repo-wide (loosens `ValidatingCodec`'s publish-time validation only — `required`/`properties`/type checks unaffected, so no previously-valid payload can now fail). (2) `mfareset.json` renamed to `mfa_reset.json` (matches schema-gov's snake_case derivation from `MFAReset`/`MFAResetPayload`); `glue_codec.go`'s `schemaFileNames` map and every other reference updated (`scripts/init-floci.sh`, `.github/scripts/stage-produced-event-schemas.sh`, `glue_codec_fs_test.go`, `glue_codec_test.go`, `test/unit/domain_test.go`). Also closed a gap from rev 2.6's own `department_catalog_changed.json` addition: it was missing its `schemaFileNames` map entry (`"department_catalog_changed.json": "DepartmentCatalogChanged"`) — harmless in practice (the event is consumed-only, never `Encode`d, so `AllSchemaNames`'s `domain.IsProducedEvent` filter already excluded its snake_case fallback stem) but inconsistent with the file's own "a new schema file needs one entry added here" convention comment; added. See the Gap 12 appendix's schema-governance caveats note for detail. |
...
**Two schema-governance caveats found while adding the AsyncAPI declaration (2026-09-20), confirmed real and fixed 2026-09-21 (rev 2.8) once a real CI run against the pinned `platform-schemagov:0.4` image hit both:**
1. Pass 5's "Open-schema guard" rejects `additionalProperties: false` on any event schema. **All 27 pre-existing schema files in `internal/adapter/outbound/eventbus/schemas/` had it** — changed to `additionalProperties: true` repo-wide. This only loosens `ValidatingCodec`'s publish-time validation (EVT-10) — a payload with unlisted extra fields is now accepted where it previously wasn't; `required`/`properties`/type checks are untouched, so no previously-valid payload can now fail.
2. `api/asyncapi.yaml` references `mfa_reset.json` (via schema-gov's own snake_case derivation from `MFAReset`/`MFAResetPayload`, not a literal filename string anywhere in the spec), but the committed schema file was named `mfareset.json` (no underscore) — traced to a real prior commit (`0f74f29 fix: sync event schemas with schema-gov extract 0.4`). Renamed the file to `mfa_reset.json`; updated `glue_codec.go`'s `schemaFileNames` map (production event-type lookup) and every other reference (`scripts/init-floci.sh`, `.github/scripts/stage-produced-event-schemas.sh`, `glue_codec_fs_test.go`, `glue_codec_test.go`, `test/unit/domain_test.go`) to match.

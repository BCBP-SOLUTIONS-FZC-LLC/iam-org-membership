...
| Rev | Date | Change |
|---|---|---|
| 2.6 | 2026-09-20 | **Production-readiness hardening pass: one new CI enforcement gate, one monitoring-gap query implemented (not yet wired), one AsyncAPI gap closed.** (1) **Database-access invariant now CI-enforced, not just documented**: added a `golangci-lint` `forbidigo` rule (`.golangci.yml`) banning `pgxpool.New`/`pgxpool.Connect`/`pgx.Connect`/`sql.Open` repo-wide — every DB connection must go through `pgcommon.NewPool`. First version of the rule (trailing `\(` in the pattern) silently matched nothing under `analyze-types: true`, which matches the resolved identifier, not call-site text; caught via a deliberate sanity-check violation before trusting it, fixed. Verified zero violations repo-wide, production and test. See `.claude/architecture.md`'s new "Database Access Rule" section. (2) **`iam_rls_violations_total` was registered but never incremented** (a known gap noted in `.claude/operations.md`) — the query side is now implemented and tested (`GaugeRepository.RLSViolationCounts`, `GROUP BY violation_type` over `rls_violation_log`), verified against real Postgres, but it is **not yet wired into a `cmd/server` exporter goroutine** — see the commit-sequencing note below. (3) **`api/asyncapi.yaml` was missing the `catalog-orgm-q`/`DepartmentCatalogChanged` channel entirely** despite `catalog_consumer.go` (Gap 12) existing and being tested — added, with a deliberately open/undefined payload schema since neither this service's consumer nor `iam-catalog-admin`'s (nonexistent) producer defines real fields yet; see the Gap 12 appendix above for the two schema-governance caveats this surfaced (Pass 5 `additionalProperties` rule failing on all 25 pre-existing schemas; a pre-existing `mfa_reset.json`/`mfareset.json` filename mismatch). Merged statement coverage independently re-verified at 99.1% (`make cover-func`) during this pass, up from the 98.4% rev-2.3 figure. **Commit-sequencing note (2026-09-21):** items (1) and (3) above landed in this repo's own commit history independently of anything else in flight; item (2)'s query method + test landed too, since it's additive and standalone. Two further things found true and tested in the working tree during this pass were **deliberately excluded** from this commit because they are entangled with a separate, still-uncommitted `platform-events`/`eventcfg` migration (noted in rev 2.5) that rewrites `cmd/reconciler/jobs/context.go`'s `Context` struct and `cmd/server/exporters.go`'s signature: (a) **removing `port.ReconcilerStore.PruneOutbox`** — a hand-rolled batched `DELETE FROM outbox_events` duplicating `outbox.Runner.PrunePublished` (§8.3/rev 2.5's fix), confirmed zero production callers once that migration lands, but its removal can't be committed standalone without breaking the build against HEAD; (b) **wiring `GaugeRepository.RLSViolationCounts` into a 5th `cmd/server` exporter goroutine** — same reason, `exporters.go`'s construction signature is being rewritten by the other migration. A companion CI script, `.github/scripts/check-outbox-access.sh`, was written and verified to fire correctly, but is likewise **not yet wired into `validate-quality.yml`** — wiring it in now would fail CI immediately, since it correctly flags `PruneOutbox`'s still-present hand-rolled SQL as a violation of the very rule it enforces. All of (a), (b), and the CI wiring are expected to land together in the same future commit as the `platform-events`/`eventcfg` migration. |
...
**One more CI-enforced invariant, added 2026-09-20 (rev 2.6), sitting alongside `go-arch-lint` but implemented differently:** every database connection/pool must go through `pgcommon.NewPool` — enforced by a `golangci-lint` `forbidigo` rule (`.golangci.yml`) banning `pgxpool.New`/`pgxpool.NewWithConfig`/`pgxpool.Connect`/`pgx.Connect`/`pgx.ConnectConfig`/`sql.Open` anywhere in the tree. Verified to actually fire (not just be present in config) against a deliberate sanity-check violation before being trusted — the rule's first draft used a trailing `\(` in the pattern, which `analyze-types: true` mode silently never matches (it resolves the qualified identifier, not call-site text). Full detail: `.claude/architecture.md`'s "Database Access Rule" section. A companion outbox-access rule (`.github/scripts/check-outbox-access.sh`, same detection approach) exists on disk but is deliberately **not yet wired into `validate-quality.yml`** — see the rev 2.6 changelog entry's commit-sequencing note.

...
| Rev | Date | Change |
|---|---|---|
| 2.10 | 2026-09-21 | **`mfa_reset.json` vs `mfareset.json` confirmed as a genuine, permanent `platform-schemagov:0.4` internal inconsistency — resolved by decision, not a fix.** Running the real pinned image directly (`make schema-validate`, three times) proved `extract`'s writer and `validate`'s Pass-7 coverage check derive *different* expected filenames for the MFAReset event from the same `asyncapi.yaml` source — no filename satisfies both. Decision (owner's call): **keep `mfa_reset.json`**, so `schema-registry.yml`'s `validate`/Pass 7 stays clean; `extract --check` will therefore always report exactly this one file as drifted. Rather than leave `validate-test.yml`'s "Event schema sync check" step permanently red for a known, understood exception, it now wraps `extract --check`: a clean run passes as before; a failure is inspected, and if the drifted-file set is *exactly* `{internal/adapter/outbound/eventbus/schemas/mfa_reset.json}` it's treated as the known exception (non-blocking `::warning::`, exit 0) — any other drift, including this file plus something else, still fails the build (verified against 4 cases: clean, known-exception-only, exception-plus-new-drift, and a different single file — only the first two pass). Running `make schema-validate` locally will still show a stray `mfareset.json` after its `extract-schemas` step and a "15 errors" `validate` result — that's the same tool inconsistency, not a new bug; delete the stray file and re-run `validate` alone (without `extract-schemas`) to see the real, expected 14-error (Pass 5 only) state. |
...
1. Pass 5's "Open-schema guard" rejects `additionalProperties: false` on any event schema — **but only 14 of the 28 events (the *consumed*, other-service-owned ones) are supposed to be open**; the other 14 (this service's own *produced* events) are deliberately CLOSED per §16 OQ-6 ("Resolved") and `asyncapi.yaml`'s own `x-forward-compatibility` block, which explicitly forbids flipping them without an org-wide decision. Rev 2.8 missed this distinction and flipped all 28 to `true`. Rev 2.9 reverted the 14 produced schemas back to `false` and, at the actual root cause, added an explicit `additionalProperties: true` to the 14 consumed schemas' `asyncapi.yaml` components (previously left implicitly unset, which `schema-gov extract` was defaulting to `false` on regeneration). Pass 5 still fails today for the 14 produced/closed schemas — an unresolved, pre-existing conflict between this service's documented architecture and current platform `schema-gov` policy, not something fixed here; see rev 2.9's changelog entry.

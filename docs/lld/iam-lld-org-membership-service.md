...
| `platform_dlq_messages_total` | 1 | Counter | `event_type`, `reason` | Events rejected to DLQ without recording `processed_events`. Currently `reason="future_time_clamp"` only (EVT-15). Canonical per the standard's own plain Tier-1 Examples — no ratification needed |

...
| `platform_duplicate_messages_total` | 1 | Counter | `consumer` | SQS redeliveries filtered by the `processed_events` composite PK (IDEMP-4) |
| `platform_dlq_messages_total` | 1 | Counter | `event_type`, `reason` | Events rejected to DLQ without recording `processed_events`. Currently `reason="future_time_clamp"` only (EVT-15) |
| `platform_dependency_request_seconds` | 1 | Histogram | `target_service` (`catalog`\|`group_mapping`\|`delegation`), `endpoint` | Latency of the synchronous cross-service client calls (`catalogadmin`/`groupmappingclient`/`delegationcheck`); `target_service` is the downstream peer, distinct from the `service` const label (this service's own identity). Direct source for the §18.7–§18.9 latency budgets |
| `platform_dependency_errors_total` | 1 | Counter | `target_service`, `endpoint`, `outcome` (`5xx`\|`timeout`\|`fallback_served`) | Cross-service call failures; `fallback_served` counts a Catalog miss served from last-known-good `om:plans`/`om:departments` (§9.3) |

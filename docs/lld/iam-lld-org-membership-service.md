...
**Two schema-governance caveats found while adding the AsyncAPI declaration (2026-09-20), unrelated to Gap 12 itself:**
1. A locally-cached `schema-gov` Docker image (not the CI-pinned `ghcr.io/.../platform-schemagov:0.4` — that registry requires auth this environment doesn't have, so it couldn't be pulled to compare) enforces a "Pass 5: Open-schema guard" rule rejecting `additionalProperties: false` on any event schema. **All 25 pre-existing schema files in `internal/adapter/outbound/eventbus/schemas/` fail this rule** — every one of them has `additionalProperties: false`. Whether this is a live, currently-failing CI gate or a preview of a newer/stricter `schema-gov` version than `:0.4` could not be determined without registry access. Worth confirming against the real pinned image before assuming either way.
2. `api/asyncapi.yaml` references `mfa_reset.json`, but the committed schema file is named `mfareset.json` (no underscore) — a genuine pre-existing mismatch, traced to a real prior commit (`0f74f29 fix: sync event schemas with schema-gov extract 0.4`), not something introduced by this pass. `schema-gov validate`'s Pass 7 (AsyncAPI↔schema coverage) flags both directions: the referenced file missing, and the actual file unreferenced.

Neither caveat was fixed here — both are pre-existing, outside Gap 12's scope, and (1) in particular is a judgment call (schema strictness policy) rather than an obvious bug fix.

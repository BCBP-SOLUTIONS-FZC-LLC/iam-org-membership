...
| Sibling LLDs | `iam-lld-catalog-admin-config-service.md`, `iam-lld-group-mapping-jit-config-service.md`, `iam-lld-tender-acl-service.md`, `iam-lld-delegation-service.md`, `iam-lld-realm-provisioner-service.md` (mints the `platform-automation` principal), `iam-lld-token-service.md` (custodies its credential; the AUTH-9 non-member guarantee here is their companion) |
...
| Rev | Date | Change |
|---|---|---|
| 2.12 | 2026-09-23 | **Event pipeline hardening — consumer Glue decode, consumed-schema validation, immediate DLQ routing, Glue version resolution by definition.** (1) **Consumer Glue decode:** every inbound queue now carries `eventbus.GlueDecoder` via `events.WithConsumerCodec` (`cmd/server/wiring.go`'s `buildSQSConsumer`) — a decode-only codec that strips the 18-byte header with no Glue client or registry. Previously an upstream envelope with a non-empty `dataschema` failed decode on every delivery and ended in the DLQ. (2) **Immediate DLQ routing** (`cmd/server/dlq.go`): permanent rejects — EVT-15 `ErrPoisonPill` (`future_time_clamp`) and consumed-schema violations (`schema_violation`) — are `SendMessage`'d to the queue's DLQ (dataschema cleared; `EventType`/`DLQReason` attributes) and the source message acked, instead of burning `maxReceiveCount` deliveries (during which EVT-15's clamp could pass once wall-clock caught up). DLQ URL resolved at startup from the queue's own `RedrivePolicy`; unresolvable DLQ or failed send falls back to normal SQS redrive. (3) **Consumed-schema validation (EVT-17, §7.1):** `cmd/server/inbound_schema.go`'s `validateConsumed` checks every decoded payload against the embedded consumed schema (`ValidatingCodec.Validate`, new `ErrNoSchema` sentinel) before `Handle`; unknown types pass through to `ackUnknown`; violations count as `platform_dlq_messages_total{reason="schema_violation"}` and page via the new `IAMConsumedEventSchemaViolation` alert. Pipeline per queue: DLQ router → `platform_messages_*` → validation → `Handle`. (4) **`TenantRealmReadyPayload` corrected in `api/asyncapi.yaml`:** `realm_id`/`realm_type` → `realm` + `keycloak_shard` (both required, `minLength: 1`), matching Realm Provisioner's producer schema; regenerated `tenant_realm_ready.json`. (5) **Glue version resolution by definition (§7.3):** `GlueCodec` resolves each produced schema's version UUID once at startup with `glue:GetSchemaByDefinition`, sending the embedded schema in exactly the compact form `schema-gov register` uploads (`registeredDefinition`, parity-tested against Python for all 28 files), requiring `AVAILABLE`. The `GetSchemaVersion(LatestVersion)` prefetch and the 5-minute `StartRefresher` are deleted: "latest" mislabelled events during deploy/registration races, rollbacks and registered-ahead-of-deploy. A definition not registered yet fails startup until `schema-registry.yml` registers it. New floci integration test (`test/integration/glue_codec_test.go`) proves a build keeps stamping its own version after a newer one is registered. No table, route or event-name change. |
| 2.11 | 2026-09-21 | **Org-wide decision made: reverse the CLOSED-schema architecture (§16 OQ-6) to OPEN, resolving the Pass-5 conflict rev 2.9/2.10 left pending.** All 14 produced payload schemas — both `api/asyncapi.yaml`'s components and the on-disk JSON files — now declare `additionalProperties: true`, matching the 14 consumed schemas and the sibling `iam-user-profile` service's convention. Rewrote `asyncapi.yaml`'s `info.description` "Schema Evolution Rules" section and the entire `x-forward-compatibility` block to describe the OPEN-schema model (backward-compatible field additions, `required-settings` per language, `open-schema-contract`, updated `ci-integration`) instead of the retired CLOSED-schema rollout protocol — mirrored on `iam-user-profile`'s own equivalent, already-open-schema `asyncapi.yaml` for consistent platform-wide language. Also fixed two small pre-existing staleness bugs found while rewriting this block: "13 payloads emitted" → 14 (MFAReset was never counted), and "iam-membership-events ... 11" → 12 (same reason). **Verified against the real pinned `platform-schemagov:0.4` image directly: all 8 validate passes now succeed cleanly for the first time this session** (previously always at least Pass 5's 14 errors); `extract --check` shows drift on exactly one file, the already-known, already-decided `mfareset.json`/`mfa_reset.json` naming exception (rev 2.10) — nothing else. §16 OQ-6 and §7.3.1's SCHEMA-5 invariant are updated to record this as a superseding decision, not a further correction to the same text (the CLOSED-schema decision was genuinely true and intentional when OQ-6 first documented it — it has now been formally reversed, with a real consumer-rollout cost the LLD's own `x-forward-compatibility` rollout-sequence already describes, since every one of the 14 producing call sites and every validating consumer must tolerate unknown fields going forward). |
| 2.10 | 2026-09-21 | **`mfa_reset.json` vs `mfareset.json` confirmed as a genuine, permanent `platform-schemagov:0.4` internal inconsistency — resolved by decision, not a fix.** Running the real pinned image directly (`make schema-validate`, three times) proved `extract`'s writer and `validate`'s Pass-7 coverage check derive *different* expected filenames for the MFAReset event from the same `asyncapi.yaml` source — no filename satisfies both. Decision (owner's call): **keep `mfa_reset.json`**, so `schema-registry.yml`'s `validate`/Pass 7 stays clean; `extract --check` will therefore always report exactly this one file as drifted, using **its own computed name (`mfareset.json`) as the reported path**, not the committed file's actual name — a subtlety this revision's first commit got wrong (it checked for `mfa_reset.json` in the drifted-file set, which never matches; corrected same day after testing directly against the real tool's output caught it). Rather than leave `validate-test.yml`'s "Event schema sync check" step permanently red for a known, understood exception, it now wraps `extract --check`: a clean run passes as before; a failure is inspected, and if the drifted-file set is *exactly* `{internal/adapter/outbound/eventbus/schemas/mfareset.json}` it's treated as the known exception (non-blocking `::warning::`, exit 0) — any other drift, including this file plus something else, still fails the build (verified against 4 cases: clean, known-exception-only, exception-plus-new-drift, and a different single file — only the first two pass). Running `make schema-validate` locally will still show a stray `mfareset.json` after its `extract-schemas` step and a "15 errors" `validate` result — that's the same tool inconsistency, not a new bug; `extract-schemas` now deletes that stray file automatically (see the Makefile fix in this same day's commits) so `schema-validate` shows the real, expected error count without manual cleanup. |
| 2.9 | 2026-09-21 | **Rev 2.8 over-corrected — it flipped all 28 schemas' `additionalProperties` to `true`, silently reversing the deliberate CLOSED-schema decision (§16 OQ-6, "Resolved") for the 14 *produced* events without the org-wide decision `asyncapi.yaml`'s own `x-forward-compatibility` block explicitly requires before that flip.** A second, real CI run (`schema-gov extract --check`, `validate-test.yml`) caught the consequence indirectly: 27 of 28 on-disk schema files showed DRIFT against what `extract` regenerates from `api/asyncapi.yaml`, because `asyncapi.yaml`'s own component schemas for the 14 *consumed* events had never explicitly declared `additionalProperties: true` — they were left implicitly unset (intending "open"), and `schema-gov extract` was defaulting unset to `false` on every regeneration, which is what actually produced the original all-`false` state Pass 5 first flagged in rev 2.8, not a uniform "everything should be true" bug. **Corrected**: the 14 produced schemas' on-disk JSON files are reverted to `additionalProperties: false`, matching `asyncapi.yaml`'s own already-`false` component declarations (untouched by this fix) — no code or source change needed there, only undoing rev 2.8's mistaken flip. The 14 consumed schemas' `asyncapi.yaml` components now explicitly declare `additionalProperties: true` (matching the pre-existing "deliberately left unset (open)" comment's *intent*, just no longer relying on an unset value schema-gov's own extractor doesn't treat as open) — both source and derived JSON verified content-identical to a fresh `schema-gov extract` (28/28 match, using a locally-cached image; the CI-pinned `:0.4` itself remains unreachable from this environment for a live re-run). **Net result**: `extract --check` (drift) and Pass 7 (AsyncAPI↔schema coverage) are now fully clean. **Pass 5 (open-schema guard) still fails for the 14 produced schemas** — this is not a bug to fix but the pre-existing, deliberate closed-schema architecture (§16 OQ-6) genuinely conflicting with a platform-wide `schema-gov` policy that apparently changed or was never reconciled with this service's documented exception. Resolving it requires the "org-wide decision" `asyncapi.yaml` itself calls for — surfaced to the user rather than decided unilaterally a second time. The `mfa_reset.json` rename from rev 2.8 stands unchanged and remains correct (a separate, already-resolved naming issue, confirmed via Pass 7 now passing clean). |
| 2.8 | 2026-09-21 | **Real CI run against the pinned `platform-schemagov:0.4` image confirmed both schema-governance caveats rev 2.6 flagged as unconfirmed.** (1) Pass 5's open-schema guard: all 27 pre-existing `internal/adapter/outbound/eventbus/schemas/*.json` files had `additionalProperties: false`; changed to `additionalProperties: true` repo-wide. **This was an over-correction — see rev 2.9**, which reverts the 14 produced/closed schemas and properly fixes only the 14 consumed/open ones at the source. (2) `mfareset.json` renamed to `mfa_reset.json` (matches schema-gov's snake_case derivation from `MFAReset`/`MFAResetPayload`); `glue_codec.go`'s `schemaFileNames` map and every other reference updated (`scripts/init-floci.sh`, `.github/scripts/stage-produced-event-schemas.sh`, `glue_codec_fs_test.go`, `glue_codec_test.go`, `test/unit/domain_test.go`) — this part was correct and stands. Also closed a gap from rev 2.6's own `department_catalog_changed.json` addition: it was missing its `schemaFileNames` map entry (`"department_catalog_changed.json": "DepartmentCatalogChanged"`) — harmless in practice (the event is consumed-only, never `Encode`d) but inconsistent with the file's own "a new schema file needs one entry added here" convention comment; added. |
| 2.7 | 2026-09-21 | **`platform-events`/`eventcfg` config-loading migration lands, plus the two items rev 2.6 deferred.** `cmd/server/wiring.go` (new) factors the topic-publisher/codec/SQS-consumer builder helpers out of `main.go`; `cmd/reconciler/jobs/context.go`'s `Context` gains an `OutboxRunner *outbox.Runner` field, wired by `cmd/reconciler/main.go`. This unblocks the two things rev 2.6 found true and tested but couldn't commit standalone without breaking the build: (1) **`port.ReconcilerStore.PruneOutbox`** — the hand-rolled batched `DELETE FROM outbox_events` duplicating `outbox.Runner.PrunePublished` — is now **removed** (interface method, implementation, and all dedicated tests/fakes deleted); `.github/scripts/check-outbox-access.sh` (written in rev 2.6, verified to fire correctly, held back pending this) is now wired into `validate-quality.yml`. (2) **`iam_rls_violations_total`** (query implemented in rev 2.6) is now wired into a 5th `cmd/server/exporters.go` goroutine (`runBusinessExporters`), mirroring `iam-user-profile`'s `runRLSViolationExporter` — the counter is live, not just registered. Also added: **`.github/scripts/check-forbidden-events-bypass.sh`** (ported from `iam-delegation`'s identically-named script, wired into `validate-quality.yml`) — guards the SNS/SQS-transport and `events.Envelope` construction boundary (no raw `aws-sdk-go-v2` SNS/SQS call outside `cmd/server/main.go`, no direct transport method call outside `internal/adapter/outbound/eventbus/`, no `events.Envelope{}` struct literal); the codebase was already compliant, this closes the CI-enforcement gap. Full batch (this migration plus the two deferred items) re-verified as one unit: `go build`/`-tags integration`/`go vet`/`gofmt` clean, `make lint` 0 issues, `arch-lint` 0 warnings, all four CI scripts pass, unit+postgres+integration+e2e suites pass with `-race`, `go mod tidy` clean, `govulncheck` 0 vulnerabilities in code, merged coverage 99.1% (well above the 95% gate). |
| 2.6 | 2026-09-20 | **Production-readiness hardening pass: two new CI enforcement gates, one dead-code removal, one monitoring gap closed, one AsyncAPI gap closed.** (1) **Database-access invariant now CI-enforced, not just documented**: added a `golangci-lint` `forbidigo` rule (`.golangci.yml`) banning `pgxpool.New`/`pgxpool.Connect`/`pgx.Connect`/`sql.Open` repo-wide — every DB connection must go through `pgcommon.NewPool`. First version of the rule (trailing `\(` in the pattern) silently matched nothing under `analyze-types: true`, which matches the resolved identifier, not call-site text; caught via a deliberate sanity-check violation before trusting it, fixed. Verified zero violations repo-wide, production and test. See `.claude/architecture.md`'s new "Database Access Rule" section. (2) **Events/outbox invariant likewise CI-enforced**: added `.github/scripts/check-outbox-access.sh` (wiring into `validate-quality.yml` completed in rev 2.7, once the removal below could land without breaking the build), scanning Go raw-string literals (not whole-line text, to avoid false-positiving on this doc's own "inserts it into outbox_events" prose) for hand-rolled SQL against `outbox_events` outside `internal/adapter/outbound/eventbus/`. (3) **`port.ReconcilerStore.PruneOutbox`** — a hand-rolled batched `DELETE FROM outbox_events` that duplicated `outbox.Runner.PrunePublished` (§8.3/rev 2.5's fix) — confirmed zero production callers once the `PrunePublished` migration landed; its removal was prepared and verified in this pass but its commit was deferred to rev 2.7 (entangled with the in-flight `platform-events`/`eventcfg` migration noted in rev 2.5). (4) **`iam_org_membership_...iam_rls_violations_total` was registered but never incremented** (a known gap noted in `.claude/operations.md`) — the query side (`GaugeRepository.RLSViolationCounts`, `GROUP BY violation_type` over `rls_violation_log`) was implemented and tested against real Postgres in this pass; wiring it into a 5th exporter goroutine was likewise deferred to rev 2.7 for the same reason. (5) **`api/asyncapi.yaml` was missing the `catalog-orgm-q`/`DepartmentCatalogChanged` channel entirely** despite `catalog_consumer.go` (Gap 12) existing and being tested — added, with a deliberately open/undefined payload schema since neither this service's consumer nor `iam-catalog-admin`'s (nonexistent) producer defines real fields yet; see the Gap 12 appendix above for the two schema-governance caveats this surfaced (Pass 5 `additionalProperties` rule failing on all 25 pre-existing schemas; a pre-existing `mfa_reset.json`/`mfareset.json` filename mismatch). Merged statement coverage independently re-verified at 99.1% (`make cover-func`) during this pass, up from the 98.4% rev-2.3 figure. |
| 2.5 | 2026-09-20 | **§3.1/§8.8.1/§11.3 `httpx` adoption claim corrected — verified stale, not just re-confirmed.** Rev 2.3 asserted (with "confirmed by direct grep") that all five outbound clients used a hand-rolled per-package `propagateTraceparent` helper and that `internal/adapter/outbound/httpx` (`httpx.NewClient`) had "zero production callers." A fresh direct grep against the current tree shows the opposite: `propagateTraceparent` does not exist anywhere in this service's outbound-client packages, and all six outbound clients (the five from rev 2.3 plus `tokenservice`, added AUTH-9/IB-3 — see rev 2.4) construct their `http.Client` via `httpx.NewClient(timeout)`, whose `otelhttp`-wrapped transport injects W3C `traceparent` and emits a client span automatically. This migration was already complete and committed before this correction was written; rev 2.3's "confirmed by grep" claim was evidently accurate only as of that date and drifted afterward without a documentation update, or was itself mistaken — either way, §3.1 (package tree + prose), the §8.8.1 Workflow-client paragraph, and §11.3's tracing paragraph are corrected to state the current mechanism. No code changed by this revision; §18/`outbound_clients.go`'s functional descriptions are unaffected (`TokenServiceClient` was already documented correctly in rev 2.4). Also noted in this pass (uncommitted, external, not part of this LLD's own scope): a `platform-events`/`eventcfg` config-loading migration is in progress in the working tree. Its `outbox-prune` CronJob migration to `outbox.Runner.PrunePublished` initially failed 3 postgres integration tests (wrong deleted-row count); root-caused same day to a test-only gap, not a production bug — the tests never wired the new `jctx.OutboxRunner` dependency the migrated job body requires, so they silently exercised its documented no-op skip path (`OutboxRunner == nil` → `Succeeded=0`, no error) rather than the real prune logic; `cmd/reconciler/main.go`'s actual production wiring already constructed `OutboxRunner` correctly and was never the problem. Fixed by wiring a real `*outbox.Runner` into the shared test helper and two standalone tests in `test/postgres/`; full suite now passes. See `.claude/operations.md` §13.1/§20.1. |
| 2.4 | 2026-09-17 | **AUTH-9: service-account non-member guarantee (companion to the automation principal — HLD §5.8/§11.7, Realm Provisioner §2.5/RP-17, Token Service).** Ported from the tender-integration design set. The per-tenant `platform-automation` automation principal must never be able to hold a role. New invariant **AUTH-9**: it is minted directly in Keycloak by the Realm Provisioner and never flows through this service's invite/registration paths (P-6/I-3), so it acquires no `tenant_memberships` row — and because both role tables carry a composite FK to `tenant_memberships` (`fk_tnr_tenant_membership`/`fk_dm_tenant_membership`, TR-8/DM-4 — "grant cannot exist without membership"), a non-member is structurally barred from every tenant/department role. Added a defense-in-depth handler reject on the membership-create (P-6/I-3) and role-grant (P-10/P-28) paths with a new `403 service_account_not_grantable` (§17). Sibling LLDs list gains `iam-lld-realm-provisioner-service.md` and `iam-lld-token-service.md`. **No schema change** (the composite-FK bar already existed); no existing invariant, endpoint, or frozen name altered — this documents and hardens an existing structural guarantee. **Implementation follow-up (IB-3, closed 2026-09-17):** the explicit reject is now built in code — a new `port.TokenServiceClient.IsServiceAccount` outbound client (fails OPEN, `TOKEN_SERVICE_BASE_URL`/`TOKEN_SERVICE_TIMEOUT_MS`, mirrors the token-service side's new TS-5 `GET /service-accounts?principal_sub=` lookup) is checked at the top of `InvitationService.Invite`/`AddFromRegister`, `DeptMembershipService.Assign`, and `MembershipService.ReconcileRoles`, returning `403 service_account_not_grantable` before any other check. A Token Service outage degrades to allow (fail-open) rather than blocking these paths — the structural FK bar remains the primary guarantee. |
| 2.3 | 2026-09-06 | **Coverage push + two drift regressions, found and fixed; §3/§17 corrected to match.** (1) Merged statement coverage raised from 80.9% to **98.4%** (CI gate floor is 95%, `.github/scripts/coverage-gate.sh`) via ~300 new test functions across nearly every package; the only non-test fallout was one test-logic bug (a test wrongly assumed `MembershipService.SetStatus`'s owner-suspend path calls `TenantRoleRepository.CountActiveOwners` **twice** — it's called once; the test was fixed, not the code). (2) **Real Makefile bug:** `internal/core/port` and `internal/adapter/outbound/httpx` both carry their own `_test.go` files that `make test`/`make test-ci`/CI never actually ran — missing from `TEST_INTERNAL_PKGS`, even though the broader `-coverpkg` glob (`./internal/...`/`./pkg/...`) claimed to instrument them; instrumentation and execution are not the same thing. Fixed by adding both packages to `TEST_INTERNAL_PKGS`. (3) **go-arch-lint regression re-fixed:** `internal/adapter/outbound/eventbus/publisher.go` had drifted back to importing `postgres` directly (for `pgadapter.TxFromContext`) — the identical Clean-Architecture violation fixed once before (moving `WithTx`/`TxFromContext` into `internal/core/port/tx_runner.go`) had only ever landed in an uncommitted working tree and was silently lost when unrelated external commits landed; re-applied identically. `.go-arch-lint.yml` also had two components fall out of sync and were re-added — `httptransport` (`internal/adapter/outbound/httpx`) and `reconciler_jobs` (`cmd/reconciler/jobs`) — plus `tools` excluded from linting; `go-arch-lint check --project-path .` is zero-violation again. (4) §3.1's shared-library snippet and prose corrected: `go.mod` actually tracks `platform-gincommon v1.3.0` / `platform-pgcommon v1.3.0` (this document still showed `v1.2.0`/`v1.2.1`); the pgcommon bump is what supplies `IsConnectionException`/`IsInsufficientResources`/`IsPgError`, now used by `wrapConnErr` (§17, `db.go`) with a hand-rolled SQLSTATE-57/58 text fallback pgcommon doesn't cover. §17's `db_unavailable`/`dependency_unavailable` rows corrected to match: `wrapConnErr` classifies SQLSTATE class 08/53 (via the new pgcommon helpers), 57/58 (hand-rolled), and `puddle.ErrClosedPool` into `db_unavailable`, not `dependency_unavailable` — the latter remains declared/status-mapped but is not currently raised by any path (a prior bug that defaulted *unrecognized* errors to it, masking real business errors under a false 503, was already fixed and stays fixed). (5) §3 corrected for the `AuthZRepository` split (`port.AuthZRepository` + `postgres.AuthZRepository.FindMembershipProjection`, the I-8 four-table join factored out of `authz_service.go` so `core/service` depends on a port, not `*pgcommon.Pool`, directly) — added to the package tree; the three stray `gincommon.PropagateHeaders` claims (§3.1, §8.8.1, §11.3) corrected to the five outbound clients' actual mechanism, each package's own local `propagateTraceparent` helper (`otel`/`propagation.NewCompositeTextMapPropagator`), confirmed by direct grep against all five `http_client.go` files. **Found, not fixed (out of scope for this pass):** `internal/adapter/outbound/httpx` (`httpx.NewClient`, an otelhttp-instrumented shared-client factory, 100% covered by its own tests) has **zero production callers** — grepped clean outside its own test file — despite existing purely to replace the five clients' hand-rolled `http.Client` + `propagateTraceparent` construction; that migration was evidently started but never finished, and this document made no claim it was done, so nothing here needed correcting on that account, but it is noted as an open item. |
| 2.2 | 2026-09-05 | **§3.3.2 `RoutingPublisher` description corrected to match the actual implementation.** Surfaced by a cross-service SNS topic-naming audit (unrelated to this fix) that happened to read the real code. The text described `events.NewRoutingPublisher` (implying a `platform-events` library type) constructed with `TopicARNs: map[string]string{...}` and a routing-key function keyed on `Envelope.Source` — none of that exists. The real `internal/adapter/outbound/eventbus.RoutingPublisher` is this service's own local type (the shared library only exposes a single-topic `NewSNSPublisher`), holds two named `events.Publisher` fields (`membership`, `tenant`) rather than a generic map, and routes each envelope via `domain.TopicForEvent(env.Type)` — keyed on event **type**, not `Envelope.Source` (`TenantCreated`/`TrialStarted` → `tenant`, everything else defaults to `membership`). `.claude/architecture.md`'s equivalent paragraph carried the identical inaccuracy and was corrected in the same pass. No code changed — this was fictional pseudo-code, not a description of a prior real design. |
| 2.1 | 2026-09-03 | **RP↔O&M alignment corrections**, found by diffing this service's actual code against Realm Provisioner's actual code (not either side's docs). (1) `TenantRealmReady`'s consumer table row (§7.1) corrected — RP's frozen payload carries `realm`/`keycloak_shard` only, never `realm_id`/`realm_type`; the consumer previously read the latter two nonexistent fields, silently blanking both columns on every real dedicated-realm event (no `COALESCE` guard on the `UPDATE`) — fixed to read `realm` and hardcode `realm_type='dedicated'`. (2) EVT-11's `iam.membership.events` count corrected from eight to nine (twelve including the two relays and the `iam.tenant.events` pair) — the row's own enumeration had silently dropped `MFAReset` (§16 OQ-8/F6) despite the event catalogue table three rows below already documenting it correctly. (3) §8.10's invite sequence diagram corrected to show the `Idempotency-Key` header on the `CreateInvitedUser` call — RP-5 hard-requires it; it was previously omitted, which would 400 every real invite once `REALM_PROVISIONER_BASE_URL` was actually set. (4) §16 OQ-9 (I-16) corrected to note RP's poll sends a sentinel `x-tenant-id` (uuid.Nil) despite being cross-tenant — `platform-gincommon`'s shared `RequireAuth` requires that header unconditionally on every `/api/v1` route, with no per-route opt-out, so omitting it entirely (the original design) 401'd every real poll. All four were previously invisible to this service's own tests, which either bypass the real middleware chain or hand-construct the wrong payload shape to match the (buggy) code rather than RP's real wire format. |
...
│       └── jobs/
│           ├── context.go                   # jobs.Context{TxRunner, repos, Logger, RealmProvisioner, ...}
...
│   │   │   ├── authz_repository.go          # AuthZRepository — I-8's four-table join port; core/service depends on this, not *pgcommon.Pool (Clean Architecture)
│   │   │   ├── cache.go
...
│           │   ├── authz_repository.go      # AuthZRepository.FindMembershipProjection — the I-8 four-table join itself (moved out of core/service/authz_service.go)
│           │   ├── db.go                    # DSNFromEnv delegates to pgcommon.ConfigFromEnv; ApplyStatementTimeout; wrapConnErr (§17)
...
│           ├── tokenservice/                # HTTP client — service-account-not-grantable check, TS-5 (Token Service, fails open, AUTH-9/IB-3)
│           ├── httpx/                       # otelhttp-instrumented shared http.Client factory — ADOPTED by all six outbound clients above (rev 2.5; corrects rev 2.3's stale "zero production callers" claim)
...
**The reconciler drives exactly 7 CronJobs** — `invitation-expiry`, `invitation-kc-cleanup`, `realm-config-sync`, `seat-overage-reconcile`, `trial-cleanup`, `outbox-prune`, `processed-events-prune` (§13.1), one file each under `cmd/reconciler/jobs/`. The delegation and tender-ACL CronJobs (`delegation-expiry`, `delegation-review`, `delegation-cleanup`, `acl-cleanup`) left with their owning services (ADR-0007, ADR-0008 §2.1). The four metric exporters (`iam_org_membership_tenant_ownerless`, `iam_org_membership_realm_sync_pending`, `iam_org_membership_seat_overage_active`, `iam_org_membership_pending_invitations_stale`) are ticker goroutines inside `cmd/server/main.go`, not CronJobs — do not double-count them against the 7.
...
`platform-pgcommon` tracks `v1.3.0` (rev 2.3 — up from `v1.2.1`, itself up from `v1.1.1` where `Config.Logger` / `migrate.Runner.Logger` became a public `domain.Logger` type instead of an internal unexported one, which is what `internal/adapter/outbound/postgres/logger_adapter.go` implements to bridge `core/port.Logger` onto it). `v1.3.0` adds `IsConnectionException`/`IsInsufficientResources`/`IsPgError` (SQLSTATE classes 08/53), which `db.go`'s `wrapConnErr` now uses alongside a hand-rolled SQLSTATE-57/58 text match pgcommon doesn't cover (§17). `platform-gincommon` tracks `v1.3.0` too (up from `v1.2.0`); nothing in this document depends on a `v1.3.0`-specific gincommon behavior.
...
The six outbound clients (`workflow`, `realmprovisioner`, `catalogadmin`, `groupmappingclient`, `delegationcheck`, `tokenservice`) add **no** new third-party dependency — each is a plain `net/http` client behind a `core/port` interface, wired at `main.go`, mesh-only over mTLS, built on the identical construction pattern: `client: httpx.NewClient(timeout)` (`internal/adapter/outbound/httpx`, an `otelhttp`-wrapped shared `http.Client` factory whose transport injects W3C `traceparent` and emits a client span automatically — **not** a hand-rolled per-package `propagateTraceparent` helper, and **not** `gincommon.PropagateHeaders` either), plus a configurable `*_TIMEOUT_MS` env var. **Corrected rev 2.5**: rev 2.3 described the opposite — a hand-built `http.Client` per package plus a local `propagateTraceparent` helper, with `httpx` as unadopted scaffolding — "confirmed by grep" at the time, but a fresh grep against the current tree finds zero `propagateTraceparent` code anywhere and `httpx.NewClient` called from all six clients; see rev 2.5's changelog entry. There is no `userprofile` client any more — deleted as dead code once delegation's OOO coordination moved out (LLD §16 OQ-5).

### 3.2 Dependency rules (enforced in CI)

Identical to `iam-user-profile`: `core/domain` → nothing external; `core/port` → `core/domain` only; `core/service` → `core/domain` + `core/port`; `adapter/*` implements `core/port`. The client-side `catalog_service.go` / `group_mapping_service.go` depend only on their `core/port` interfaces (`catalog_reader`, `group_mapping_client`), never on the `catalogadmin` / `groupmappingclient` HTTP adapters directly — the same inward-pointing discipline every sibling service applies to an outbound dependency. Enforced by `go-arch-lint` (`.go-arch-lint.yml`), which additionally declares `httptransport` (`internal/adapter/outbound/httpx`, §3.1) and `reconciler_jobs` (`cmd/reconciler/jobs`) as their own components and excludes `tools` from linting. `internal/adapter/outbound/eventbus/publisher.go` importing `postgres` directly (for `pgadapter.TxFromContext`) is the one violation this rule set has caught twice — fixed once by moving `WithTx`/`TxFromContext` into `internal/core/port/tx_runner.go` (`postgres.WithTx`/`postgres.TxFromContext` remain as thin re-export wrappers for existing test call sites), lost when that fix landed only in an uncommitted working tree and was silently overwritten by unrelated external commits, then re-applied identically (rev 2.3). `go-arch-lint check --project-path .` currently reports zero violations; re-verify this specific import after any rebase that touches `eventbus/publisher.go`.

**Three more CI-enforced invariants, added 2026-09-20/21 (rev 2.6/2.7), sitting alongside `go-arch-lint` but implemented differently:** (1) every database connection/pool must go through `pgcommon.NewPool` — enforced by a `golangci-lint` `forbidigo` rule (`.golangci.yml`) banning `pgxpool.New`/`pgxpool.NewWithConfig`/`pgxpool.Connect`/`pgx.Connect`/`pgx.ConnectConfig`/`sql.Open` anywhere in the tree; (2) every `outbox_events` mutation must go through `platform-events` (`outbox.Enqueue`/`outbox.Runner.PrunePublished`), never hand-rolled SQL — enforced by `.github/scripts/check-outbox-access.sh` (wired into `validate-quality.yml` in rev 2.7, once `port.ReconcilerStore.PruneOutbox`'s removal could land without breaking the build); (3) every SNS publish / SQS consume must go through `platform-events`, never a raw `aws-sdk-go-v2` SNS/SQS call or a hand-built `events.Envelope{}` — enforced by `.github/scripts/check-forbidden-events-bypass.sh` (rev 2.7, ported from `iam-delegation`). All three were verified to actually fire (not just be present in config) against deliberate sanity-check violations before being trusted — the `forbidigo` rule's first draft used a trailing `\(` in the pattern, which `analyze-types: true` mode silently never matches (it resolves the qualified identifier, not call-site text). Full detail: `.claude/architecture.md`'s "Database Access Rule" and "Events / Outbox / Dedup Rule" sections.
...
`internal/adapter/outbound/eventbus.RoutingPublisher` (HLD §9.2) is this service's **own local wrapper**, not a `platform-events` type — v1.3.0/v1.4.0 exposes `NewSNSPublisher` for a single topic only, so a two-topic producer has to wrap it locally. `RoutingPublisher` holds two pre-built `events.Publisher` fields (`membership`, `tenant`), not a generic topic-ARN map; `NewRoutingPublisher(membership, tenant events.Publisher)` wires them at composition-root time in `cmd/server/main.go`, and either may be `nil` in dev to disable that lane — an envelope routed to a `nil` lane returns a descriptive error rather than being silently dropped. Routing is per-envelope via **`domain.TopicForEvent(env.Type)`** — an explicit switch keyed on the event **type**, not `Envelope.Source`: `EventTenantCreated`/`EventTrialStarted` route to the `tenant` publisher; every other type defaults to `membership`. `PublishBatch` groups envelopes by topic first so a same-topic batch still uses the underlying publisher's own batch API rather than falling back to one `Publish` call per envelope. This is the one meaningful difference from the User Profile wiring. (Delegation events no longer route from here — they are produced by `iam-delegation` on `iam.delegation.events`, ADR-0008 §10.)
...
-- and the iam_org_membership_realm_sync_pending gauge scan (§11.2) are index-only over the handful of rows awaiting realm convergence.
...
-- (§16 A59) Small partial index over the over-cap set only, so the iam_org_membership_seat_overage_active gauge scan (§11.2) and the
...
| T-13 | `ownerless_since` (§16 A39) is NULL for every tenant with an owner and is set to `now()` **only** by the TM-12 complete-and-escalate path (I-5 removing the last active `tenant_owner`). It is **cleared only by O-7** (operator owner-reassignment). A non-NULL value means the tenant currently has **zero** active owners and needs operator intervention — it is the durable backing for the `iam_org_membership_tenant_ownerless` alert (§11.2), so a transient set/clear must never be used for anything else. O&M-originated (not a Billing/Realm projection); no bus event is emitted for the state (escalation is via the metric/alert + audit log, deliberately avoiding a new HLD event-catalog entry, §16 A39). |
...
| SEAT-5 | **`overage_since` is the durable seat-overage marker and grace clock (§16 A59).** `timestamptz`, NULL when `active_memberships + pending_invitations ≤ licensed_seats`. It is set to `now()` when that sum **first exceeds** `licensed_seats` — reachable **only** via a Billing-driven `licensed_seats` **decrease** (SEAT-2), because SEAT-1 prevents usage from ever growing past the cap — and **cleared** when usage returns to at/under cap. It is (re)evaluated inside the **tenant row lock** by every transaction that changes a SEAT-1 triple term (a `TenantSeatsChanged` consume, a member add/remove P-6/P-8/I-5, and an invite create/expire/revoke/accept P-6/expiry-cron/P-31/I-3), with a daily **`seat-overage-reconcile`** backstop (§13.1) that recomputes it for the `idx_tenants_seat_overage` set to self-heal any missed set/clear. On the NULL→set transition O&M emits **`TenantSeatOverageStarted`** and on set→NULL **`TenantSeatOverageResolved`** (§7.3); it drives the exposed `grace_ends_at` (§5.4) and the `iam_org_membership_seat_overage_active` gauge (§11.2). Like `ownerless_since` (T-13) it carries **no CHECK** — it is derived from live counts, not a status enum, so it is logic-maintained rather than constraint-maintained. **O&M never removes or suspends a user off this marker** — it is an observability + hand-off signal, and enforcement past grace is Billing's (SEAT-3/SEAT-4). |
...
| **+ I-16** (new subscription-lapse bulk read, §16 OQ-9/RP-C3) | 21 | 13 | 2 | 0 | **36** |
...
- **And 1 more still:** Internal **I-16** `GET /api/v1/internal/subscription-lapses` — the bulk read RP's own subscription-lapse sweep (RP-C3) polls in place of tracking `cancelled_at` itself (§16 OQ-9). `35 + 1 = 36`.

**Net retained Core public/internal/operator endpoint-ID count = 35 + I-16 = 36.** Retired IDs **I-6/I-7** (§16 A26) remain in the catalogue as struck-through historical rows and are not counted among active endpoints; extracted IDs are deleted, not renumbered — surviving IDs keep their monolith numbers.
...
| AUTH-9 | **Service-account subjects are non-members and non-grantable — the O&M half of the reserved-principal guarantee (HLD §5.8/§6.1/§11.7, Realm Provisioner §2.5/RP-17, Token Service).** The per-tenant automation principal (the `platform-automation`, `service_account`-typed Keycloak client) is minted directly by the Realm Provisioner and **never flows through this service's invite/registration paths** (P-6, I-3), so it acquires **no `tenant_memberships` row**. Because every role grant carries a composite FK to `tenant_memberships` (`fk_tnr_tenant_membership` on `tenant_roles` (TR-8), `fk_dm_tenant_membership` on `dept_memberships` (DM-4) — "grant cannot exist without membership"), a non-member principal is **structurally unable to hold any tenant-level or department-level role** — no `tenant_admin`/`tender_admin`/`approver`, and no tenant-admin action can grant it one. As defense-in-depth, the membership-create (P-6/I-3) and role-grant (P-10/P-28) handlers additionally **reject a subject that resolves to a `service_account`-typed Keycloak principal** with `403 service_account_not_grantable` before any write. The "automation can never approve, and a tenant cannot raise it" guarantee is thus enforced by the principal's *type* plus the membership FK, never by a configuration value. |
...
| **I-16** | `GET /api/v1/internal/subscription-lapses` | **Realm Provisioner (RP-C3)** | **Bulk subscription-lapse read (new, §16 OQ-9/RP-C3).** Returns every tenant with `status='cancelled'` whose `cancelled_at` is older than `SUBSCRIPTION_GRACE_DAYS` (default 30, §12) — `{ tenants: [{ tenant_id, realm_id, realm_type, cancelled_at }] }`. RP's own lapse sweep polls this in place of tracking cancellation timestamps itself, since O&M's `iam.tenant.events` production carries no cancellation data. Cross-tenant by design — the **only** internal route not scoped under `/tenants/:id`, and the only one served against the BYPASSRLS `sysPool` rather than the RLS-scoped app pool (same pool the reconciler jobs and business-metric exporters already use). O&M computes the grace-period math so the rule is single-sourced. Self-idempotent: a tenant RP has suspended drops off the next poll on its own. | no (cross-tenant; not cacheable per-tenant) |
...
1a. THROTTLE — abuse guards, checked HERE (before the RP call) so a refused invite creates NO Keycloak
    ...
    extra through — the hard seat cap in step 3 is the only correctness gate):
      ...
      Every refusal here increments iam_org_membership_invite_throttled_total{reason} (§11.2). No RP call, no email.
...
#### (I-16) `GET /api/v1/internal/subscription-lapses` — subscription-lapse bulk read (new, §16 OQ-9/RP-C3)

**Auth:** in-mesh service caller only (IAPI-1); no JWT, no tenant-user role — same mesh-mTLS-only boundary as I-15. The consumer is the Realm Provisioner's own lapse sweep (RP-C3, §8.8 of the Realm Provisioner LLD).

**Why it exists.** RP-C3 emits `TenantSuspended(source=billing_lapse)` once a `cancelled` tenant's grace period elapses (T-11/T-16, §16 OQ-7) — but RP has no way to learn `cancelled_at` for any tenant: O&M's only production onto `iam.tenant.events` (the one topic RP partially consumes, via `tenant-realm-q`) is `TenantCreated`/`TrialStarted`. RP reported this as a genuine blocker, not a missing implementation detail on their side — `SubscriptionStatusReader`'s own naming already assumed a poll/query pattern, matching this LLD's own precedent (I-11, I-15) for cross-service reads, but O&M had never published the endpoint.

**Deliberately pull, not push — unlike RP-4's resolution (§18.3) for trial-expiry.** RP-4 established that O&M exposes *no* batch "expired trials" endpoint: RP seeds `trial_ends_at` once (via `TrialStarted`) and sweeps it locally forever after, because trial expiry is strictly one-directional. Subscription cancellation is **not** — a customer can resubscribe before grace elapses (`TenantReactivated`), which a push model would have to represent as a second event RP must consume and reconcile against its own locally-cached state, with the attendant missed-delivery/out-of-order risk. A pull model sidesteps that entirely: O&M's `tenants` row already is the single source of truth, so RP simply asks it fresh on every sweep instead of keeping a shadow copy that could drift.

**Request:**

```
GET /api/v1/internal/subscription-lapses
// No x-tenant-id — cross-tenant by design, the only internal route that is.
```
...
{
  "tenants": [
    { "tenant_id": "t-uuid", "realm_id": "acme", "realm_type": "dedicated", "cancelled_at": "2026-07-15T00:00:00Z" }
  ]
}
```

Every tenant with `status = 'cancelled'` and `cancelled_at <= now() - SUBSCRIPTION_GRACE_DAYS` (default 30, §12) — O&M does the grace-period arithmetic itself so the rule is single-sourced rather than living in two independently-configured services. `realm_id`/`realm_type` are included so RP can act (suspend the realm) without a second lookup per tenant. Ordered by `cancelled_at ASC` (oldest lapses first); an empty `tenants` array is a normal response, not an error.

**Cross-tenant pool binding (the one thing that makes I-16 different from every other internal route).** Every other route in this catalogue is scoped under `/tenants/:id` and runs against the RLS-scoped app pool with `x-tenant-id` bound to the target tenant (RLS-5). I-16 has no single target tenant — it must see across all of them — so its repository method (`TenantRepository.ListSubscriptionLapses`) is constructed against the **BYPASSRLS `sysPool`** (`org_membership_migrator`/superuser), the exact same pool the reconciler jobs and the five business-metric exporter goroutines (4 gauges + the `iam_rls_violations_total` counter, added 2026-09-20 — rev 2.6) already use for cross-tenant reads (`cmd/server/main.go`). Calling it against the RLS-scoped app pool would silently narrow the result to at most one row — the caller's own tenant — rather than failing loudly, which is why the port doc comment calls this out explicitly.

**Self-idempotent — no ack, no cursor.** Once RP acts on an entry (emitting `TenantSuspended`, which flips O&M's `status` to `suspended`), that tenant no longer matches `status = 'cancelled'` and drops off the very next poll on its own. There is nothing for RP to acknowledge back to O&M and nothing for O&M to track per-caller.

**Authz:** internal / mTLS-only (IAPI-1); no JWT, no tenant-user role. The system principal is accepted here (AUTH-5).
...
| `TenantRealmReady` | Update `tenants.keycloak_realm` (`realm_id`) with the provisioned realm name, set `realm_type='dedicated'` (§16 A22), **and record `keycloak_shard`** (§16 A23, T-12) — all in the same `UPDATE`, so realm name, strategy, and placement never observably disagree. The shard value is chosen by the Realm Provisioner (O&M stores it, never selects it); at MVP it is `'shard-0'`. **Wire-format correction:** RP's frozen `TenantRealmReadyPayload` carries the realm name under `json:"realm"` (not `"realm_id"`) and has **no** `realm_type` field at all — a prior version of this consumer read those two nonexistent field names, and since the `UPDATE` carries no `COALESCE` guard, every real occurrence silently blanked both `realm_id` and `realm_type` to empty strings (only `keycloak_shard` landed correctly). Fixed: the consumer reads `realm` into `realm_id` and hardcodes `realm_type='dedicated'` rather than reading a field RP never sends — safe because this event is, per RP's own frozen payload doc comment, "emitted by RP-2 or RP-3 (never for trial)," i.e. always a dedicated-realm event (RP-INV-2). **Contract aligned (rev 2.12):** `api/asyncapi.yaml`'s `TenantRealmReadyPayload` (and the regenerated `tenant_realm_ready.json`) previously still declared `realm_id`/`realm_type`; it now declares `realm` and `keycloak_shard`, both required with `minLength: 1`, matching RP's producer schema (RP's `tenant_id`/`oidc_clients` pass through the open schema) — so consumed-schema validation (§7.1 "Inbound pipeline") rejects a payload missing either. |
...
**Future-time clamp — poison-pill guard on the recency high-water mark (§16 A40, EVT-15).** Because the guard trusts the producer's wall-clock `time` and advances `last_event_at` to it, a **single mis-stamped producer** (a clock skewed far forward by NTP failure, a container clock drift, or a bad replay tool) could stamp one event with a far-future `time` — which, once applied, would push `last_event_at` into the future and cause **every subsequent correctly-stamped event to be silently skipped** as "stale" until wall-clock catches up, silently **freezing the tenant's projection**. That is the well-known failure mode of last-writer-wins on a wall clock, and note (b)'s "NTP-bounded" reasoning is the happy path, not the failure path. To bound the blast radius, the guard adds a **sanity clamp applied before the recency comparison**: if `event.time > now() + MAX_LIFECYCLE_EVENT_SKEW_SECONDS` (config, default 300 s — comfortably above real NTP skew, §12), the event is treated as **corrupt, not stale** — it is **not applied, `last_event_at` is not advanced, and it is NOT recorded in `processed_events`**; instead the handler **rejects it straight to the DLQ** (returns `ErrPoisonPill`; `cmd/server/dlq.go`'s router `SendMessage`s the envelope to the queue's DLQ with `DLQReason=future_time_clamp` and acks the source message — see "Inbound pipeline" below; falls back to redelivery → DLQ after `maxReceiveCount`, EVT-5, only if the DLQ is unresolvable or the send fails) and increments `platform_dlq_messages_total` (§11.2), which pages (a future timestamp is never legitimate — the producer stamps `time` at emit, so it can never validly exceed now by more than transmission latency ≪ 300 s). Failing **loud to the DLQ** rather than applying is the deliberate choice: it surfaces the mis-stamped producer for a human to fix while **never letting one bad clock poison the high-water mark** and wedge the projection. Deferred, not chosen: switching the ordering key from wall-clock `time` to a **producer-supplied monotonic version** per aggregate would remove the skew dependence entirely, but requires a producer-contract change across Billing and the Realm Provisioner; the clamp is the low-cost consume-side guard that closes the poison-pill without that coordination (§16 A40).

**Inbound pipeline (rev 2.12) — Glue decode, consumed-schema validation, immediate DLQ routing.** Every inbound queue (`tenant-orgm-q`, `billing-orgm-q`, `catalog-orgm-q`) is built by `cmd/server/wiring.go`'s `buildSQSConsumer` and composed by `cmd/server/inbound_schema.go`'s `inboundHandler`. Per delivery:

1. **Envelope parse + Glue decode.** `buildSQSConsumer` always appends `events.WithConsumerCodec(eventbus.GlueDecoder{})`. `GlueDecoder` is a decode-only `events.Codec` — no Glue client, no registry — whose `Decode` strips the self-describing 18-byte header (`[0x03][0x00][16-byte version UUID]`); its `Encode` always errors (never wired on a publisher). platform-events only calls it when `envelope.dataschema` is non-empty, so plain-JSON producers bypass it. Before this, any upstream envelope Glue-encoded by its producer (non-empty `dataschema`) failed decode on every delivery and ended in the DLQ.
2. **DLQ router** (`cmd/server/dlq.go`, `withDLQRouting` → `routeRejectsToDLQ`, outermost wrapper). platform-events treats every handler error as retryable, so a permanent reject would otherwise burn `maxReceiveCount` deliveries — and EVT-15's clamp, re-evaluated per delivery, could accept a skewed event once wall-clock caught up. `dlqReason` classifies two permanent rejects: `ErrPoisonPill` → `future_time_clamp`, `errSchemaViolation` → `schema_violation`. On either, the already-decoded envelope is `SendMessage`'d to the DLQ with `dataschema` cleared (so a later redrive isn't decoded twice) and `MessageAttributes` `EventType` + `DLQReason`, then the source message is acked. The DLQ URL is resolved once at startup from the queue's own `RedrivePolicy` (`GetQueueAttributes` → `deadLetterTargetArn`) — no new env vars, can't drift from infra. Fallbacks: DLQ unresolvable → warn log + unwrapped handler (normal SQS redrive after `maxReceiveCount`); `SendMessage` fails → original error returned (normal retry + redrive). Every other error passes through unchanged (transient → retry/redrive).
3. **`platform_messages_*` counters** (`instrumentedHandler`) — inside the router, so rejects still count as failed.
4. **Consumed-schema validation** (`validateConsumed`). The decoded payload is checked against the **embedded** consumed schema for `env.Type` — the same compiled `schemas/*.json` `ValidatingCodec` uses for produced events, via `ValidatingCodec.Validate`. No Glue call on the receive path, so it keeps working through a Glue outage; it validates this service's own contract (what `api/asyncapi.yaml` says it relies on), not the producer's registry schema. Consumed schemas are OPEN, so new producer fields pass; they were checked against Realm Provisioner's producer schemas (looser or equal) — the Billing and Catalog producer schemas were not available to verify. A type with no embedded schema (`ErrNoSchema`) passes through to `ackUnknown`. A violation increments `platform_dlq_messages_total{reason="schema_violation"}`, logs a warning, and returns `errSchemaViolation` → DLQ, never reaching the projection or `processed_events`.
5. **`Handle`** (`MembershipEventConsumer`/`CatalogConsumer`) — dedup, EVT-14/15/16 as above.
...
The service publishes to two SNS topics via its own local `eventbus.RoutingPublisher` (§3.3.2 — not a `platform-events` type):
...
| **5 — Schema shape** | The platform-wide `schema-gov` open-schema guard: any schema declaring `additionalProperties: false` is rejected outright, with no per-service exception mechanism. **This service's schemas are OPEN** (`additionalProperties: true` on all 14 produced payloads, plus `api/asyncapi.yaml` itself — §16 OQ-6, rev 2.11), matching `iam-user-profile`'s convention. This service briefly declared a CLOSED-schema policy instead (documented at length in `asyncapi.yaml`'s `info.description`/`x-forward-compatibility` blocks at the time); that decision was reversed 2026-09-21 once real CI runs against the pinned `platform-schemagov:0.4` image confirmed Pass 5 hard-blocks closed schemas with no exception, making the two policies irreconcilable in practice. |
...
| SCHEMA-5 | **Event schemas are OPEN (§16 OQ-6, rev 2.11) — reversed from a CLOSED-schema decision that stood from this LLD's rev ~2.3 through rev 2.10.** Every one of the 14 produced payload schemas, on disk and in `api/asyncapi.yaml`, now sets `additionalProperties: true`: consumers ignore unknown fields, adding an optional field is a non-breaking, no-rollout-required change (only removing/renaming a field, changing type/format, narrowing an enum, or widening required fields are breaking, per `asyncapi.yaml`'s `x-forward-compatibility` block). Consumers MUST enable lenient SDK-level deserialization (per `asyncapi.yaml`'s `required-settings`) before the producer starts emitting new fields, or they will crash — Go strict-deserializer scanning (Pass 6) guards this on our own side. Reversed 2026-09-21 once real CI runs against the pinned `platform-schemagov:0.4` image confirmed its Pass 5 (open-schema guard) hard-blocks any closed schema with no per-service exception — an org-level conflict the earlier CLOSED-schema decision could not coexist with in practice. |
...
**Runtime codec — `platform-events` v1.4.0's `events.Codec`, adopted (§16 A68; supersedes the earlier "deferred, catalog-only" text here).** Glue is no longer CI-catalog-only: `internal/adapter/outbound/eventbus/glue_codec.go` implements `events.Codec` and is wired per topic.
- **Publish side.** The outbox always stores plain, validated JSON (`ValidatingCodec` at enqueue); `GlueCodec` is applied transiently by the SNS publisher (`events.WithCodec`, one instance per registry/topic, `NoopCodec` when `GLUE_REGISTRY_*_NAME` is unset) — it prepends the 18-byte header `[0x03][0x00][16-byte version UUID]` and echoes the UUID into `envelope.dataschema`. **Version resolution is by definition, not latest (rev 2.12):** at startup, for each of the 14 produced schemas, `NewGlueCodec` calls `glue:GetSchemaByDefinition` with this binary's embedded schema in **exactly** the string `schema-gov register` uploads — Python `json.dumps(schema, separators=(",", ":"))`: compact, key order preserved, non-ASCII `\uXXXX`-escaped (`registeredDefinition` = `json.Compact` + `asciiEscape`) — and requires `Status = AVAILABLE`. The UUID is then fixed for the process lifetime; there is no refresher (the former `GetSchemaVersion(LatestVersion)` lookup, `StartRefresher` 5-minute ticker and `WithLogger` are deleted). "Latest" was wrong during deploy/registration races, after rollbacks, and when a version is registered ahead of its deploy; by-definition always stamps the version that actually describes the payload. A definition that isn't registered yet fails startup — `` resolve glue schema "<name>" in registry "<reg>" by definition: <err> — this binary's schema version isn't registered yet: wait for schema-registry.yml to register it, or run `make schema-verify` `` — so the pod CrashLoops until `schema-registry.yml` registers it, then self-heals. Zero Glue calls per event.
- **Consume side.** `eventbus.GlueDecoder` (decode-only, no Glue client/registry) on every inbound queue via `events.WithConsumerCodec` — see §7.1 "Inbound pipeline". Zero Glue calls per event here too.
- **Cross-service note.** Downstream consumers of `iam.membership.events`/`iam.tenant.events` must likewise wire `WithConsumerCodec` (a header-stripping decoder suffices) wherever this service runs with a Glue registry configured, or they receive header-prefixed bytes; still tracked under §16 A68 (recommend-and-confirm, same posture as §16 A32(i)).
...
**The canonical spec is referenced here, not reproduced.** Embedding or paraphrasing its contents in this document would duplicate it and invite drift. Consult the file directly for the complete channel/operation/message/schema catalogue (14 produced event types across both topics, plus 14 consumed), the consumer channels and `consumeTenantOrgm`/`consumeBillingOrgm`/`consumeCatalogOrgm` operations, the `info.x-*` governance blocks, and the published/consumed tag grouping; §7.3.1 above covers Glue registry specifics.
...
| EVT-5 | **DLQ after 5 attempts.** A message that fails `maxReceiveCount = 5` delivery attempts is moved to the queue's `<queue>-dlq` (`tenant-orgm-q-dlq`, `billing-orgm-q-dlq`) for out-of-band handling — it is not retried indefinitely or silently dropped. **Permanent rejects skip the retry budget** (rev 2.12): an EVT-15 clamp or an EVT-17 schema violation is sent to the DLQ on first delivery (`DLQReason` message attribute = `future_time_clamp`/`schema_violation`) and the source acked, via `cmd/server/dlq.go` — DLQ resolved from the queue's own `RedrivePolicy`; unresolvable DLQ or failed send falls back to this 5-attempt redrive. |
...
| EVT-11 | **Only declared event types are published.** O&M emits only the event types defined in `api/asyncapi.yaml`: the **nine** `iam.membership.events` — `DepartmentMembershipGranted`/`Revoked`/`LevelChanged`, `TenantRoleGranted`/`TenantRoleRevoked`, `MembershipRevoked` (new, ADR-0008), `TenantMembershipsPurged` (new, ADR-0008), `TenderAssigneeOverridden`, `MFAReset` (new, §16 OQ-8/F6, P-34's sole audit signal) — plus the two relays `TenantStateChanged` and the `TenantSeatOverage*` pair, plus `TenantCreated` / `TrialStarted` on `iam.tenant.events` (**twelve** total across both topics); ad-hoc or undeclared event schemas are not permitted — CI (`schema-gov`) fails on drift (SCHEMA-1…SCHEMA-6, §7.3.1). (**Count history:** seven → eight (rev 0.98, `TenantRoleRevoked`, §16 A14) → nine (rev 1.69, `DelegationReviewRequested`, §16 A70) → eight under ADR-0008 (the three `Delegation*` types removed to the Delegation Service's `iam.delegation.events`, `MembershipRevoked` + `TenantMembershipsPurged` added — net −3 +2) → **nine** with `MFAReset` added (§16 OQ-8/F6, P-34) — corrected here, this row previously omitted `MFAReset` from its own count despite the event catalogue table below (§7.3) already documenting it correctly.) |
| EVT-14 | **Projection recency — last-writer-wins guards against reordered/stale lifecycle events (§16 A33).** Every `tenant-orgm-q` / `billing-orgm-q` handler compares the event's CloudEvents `time` against `tenants.last_event_at` (under the tenant row lock, in the apply `RunInTx`): an event with `time <= last_event_at` is **skipped** (state unchanged; still recorded in `processed_events`; `iam_lifecycle_event_skipped_total`++), and a newer event **applies its change and advances `last_event_at = event.time`**. This makes the `tenants` projection **order-independent** — the newest producer-stamped event wins regardless of SNS/SQS delivery order — closing the gap that `processed_events` (exact-duplicate only, EVT-4) and EVT-6 (legal-transition only) leave open. Complements, does not replace, EVT-6: EVT-6 rejects an *illegal* transition; EVT-14 rejects a *stale* one. `last_event_at` is advanced only by consumed events, never by tenant-facing API writes. The recency comparison runs **after** EVT-15's future-time clamp, so a corrupt far-future timestamp can never advance the high-water mark. |
...
| EVT-15 | **Future-time clamp — the recency guard fails loud, never poisons (§16 A40).** Because EVT-14 advances `last_event_at` to the producer's wall-clock `time`, one mis-stamped far-future event would otherwise freeze the whole projection (every later, correctly-stamped event would read as stale). So **before** the EVT-14 comparison, any event with `time > now() + MAX_LIFECYCLE_EVENT_SKEW_SECONDS` (config, default 300 s) is treated as **corrupt, not stale**: it is **not applied, does not advance `last_event_at`, and is not recorded in `processed_events`** — it is **rejected straight to the DLQ** (`ErrPoisonPill` → `cmd/server/dlq.go` `SendMessage`s it to `<queue>-dlq` with `DLQReason=future_time_clamp` and acks; redelivery → `maxReceiveCount`, EVT-5, only as the fallback) and increments `platform_dlq_messages_total{reason="future_time_clamp"}` (pages). Immediate routing matters because the clamp is re-evaluated per delivery — a retried skewed event would otherwise be accepted once wall-clock caught up. A future `time` is never legitimate (producers stamp at emit), so this only ever catches a broken clock or replay tool. Rejecting loud to the DLQ — rather than applying, or silently skipping — bounds the blast radius of a bad producer clock to one paged, replayable message instead of a wedged tenant. Switching to a producer-supplied monotonic version would remove the wall-clock dependence entirely but needs a cross-service contract change; deferred (§16 A40). |
| EVT-17 | **Consumed payloads are validated against this service's own embedded schema before `Handle` (rev 2.12).** `cmd/server/inbound_schema.go`'s `validateConsumed` checks every decoded inbound payload against the embedded consumed schema for `env.Type` (`ValidatingCodec.Validate`, same compiled `schemas/*.json` as produced events; no Glue call). OPEN schemas, so additive producer changes pass. A violation is permanent → `platform_dlq_messages_total{reason="schema_violation"}`++ (pages, `IAMConsumedEventSchemaViolation`) and straight to the DLQ; never applied, never recorded in `processed_events`. An unmodelled type (`ErrNoSchema`) passes through to `ackUnknown`. |
...
**HTTP adapter** (`adapter/outbound/workflow/http_client.go`), same construction pattern as the other five outbound clients (`realmprovisioner`, `catalogadmin`, `groupmappingclient`, `delegationcheck`, `tokenservice`) — `httpx.NewClient(timeout)` for W3C trace propagation (**not** a local `propagateTraceparent` helper, **not** `gincommon.PropagateHeaders` either, corrected rev 2.5 — see §3.1), configurable timeout (`WORKFLOW_TIMEOUT_MS`, §12):
...
sequenceDiagram
    ...
    alt active_workflows == 0
        ...
        OrgMembership->>OrgMembership: log delegate_removal_blocked {tenant_id, user_id, workflow_count} then iam_org_membership_delegate_removal_blocked_total++
...
sequenceDiagram
    ...
    OrgMembership->>RealmProv: POST /api/v1/internal/tenants/:id/users {email, full_name, required_actions}, Idempotency-Key header %% F8: versioned + tenant-nested, RP LLD v0.22. F5: required_actions computed by O&M from initial_tenant_roles/initial_dept_mappings, applied verbatim by RP (RP-5). Idempotency-Key is a hash of (tenant_id, email) - RP-5 requires this header (400 missing_idempotency_key otherwise) - a prior version of this call omitted it, which failed every real invite once RealmProv was actually reachable, masked in dev by a baseURL-empty fallback
...
Per the IAM Platform Observability Standard's three-tier hierarchy. `domain`/`service`/`environment` are injected centrally in `metrics.Register(environment)` (ConstLabels), never left to an individual call site.

- **Tier 1 — `platform_*`** (required labels `domain`, `service`, `environment`): a concept common across domains (IAM, Workflow, Billing, Tender Management, ...) with identical semantics.
- **Tier 2 — `iam_*`** (required labels `service`, `environment`, no `domain`): a concept shared across IAM-domain services but not meaningful outside IAM.
- **Tier 3 — `iam_org_membership_*`**: unique to this service's own business logic — no other IAM service has an equivalent concept.

| Metric | Tier | Type | Labels | Description |
|---|---|---|---|---|
| `platform_messages_received_total` | 1 | Counter | `queue` | Inbound SQS message dequeued, before processing — wired around both `tenant-orgm-q`/`billing-orgm-q` in `cmd/server/main.go`'s `instrumentedHandler`, not inside `MembershipEventConsumer.Handle` itself (queue identity is only known at the subscription call site) |
| `platform_messages_processed_total` | 1 | Counter | `queue` | Inbound SQS message whose `Handle` returned nil |
| `platform_messages_failed_total` | 1 | Counter | `queue` | Inbound SQS message whose `Handle` returned an error (includes DLQ rejections) |
| `platform_dlq_messages_total` | 1 | Counter | `event_type`, `reason` | Events rejected straight to DLQ without recording `processed_events` (`cmd/server/dlq.go`). `reason` = `future_time_clamp` (EVT-15) or `schema_violation` (consumed payload fails its embedded schema, EVT-17). Canonical per the standard's own plain Tier-1 Examples — no ratification needed |
| `platform_duplicate_messages_total` | 1 | Counter | `consumer` | SQS redeliveries filtered by the `processed_events` composite PK (IDEMP-4). **Status: Proposed** (`registry.go`) — shadow-emitted only; `iam_org_membership_processed_events_duplicates_total` is authoritative until ratified |
| `platform_dependency_request_seconds` | 1 | Histogram | `target_service` (`catalog`\|`group_mapping`\|`delegation`), `endpoint` | Latency of the synchronous cross-service client calls (`catalogadmin`/`groupmappingclient`/`delegationcheck`); `target_service` is the downstream peer, distinct from the `service` const label (this service's own identity). **Status: Proposed** — shadow-emitted only; `iam_org_membership_dependency_call_duration_seconds` is authoritative until ratified. Direct source for the §18.7–§18.9 latency budgets |
| `platform_dependency_errors_total` | 1 | Counter | `target_service`, `endpoint`, `outcome` (`5xx`\|`timeout`\|`fallback_served`) | Cross-service call failures; `fallback_served` counts a Catalog miss served from last-known-good `om:plans`/`om:departments` (§9.3). **Status: Proposed**, and unlike the two rows above, not even one of the standard's own named examples (added by symmetry, `registry.go`). Shadow-emitted only; `iam_org_membership_dependency_call_failures_total` is authoritative until ratified |
| `iam_org_membership_dependency_call_duration_seconds` | 3 | Histogram | `target_service`, `endpoint` | Tier-3 predecessor of `platform_dependency_request_seconds` (Proposed) — this repo's actual alerting/SLO source, dual-emitted from the same call site |
| `iam_org_membership_dependency_call_failures_total` | 3 | Counter | `target_service`, `endpoint`, `outcome` | Tier-3 predecessor of `platform_dependency_errors_total` (Proposed) — authoritative, dual-emitted from the same call site |
| `iam_org_membership_processed_events_duplicates_total` | 3 | Counter | `consumer` | Tier-3 predecessor of `platform_duplicate_messages_total` (Proposed) — authoritative, dual-emitted from the same call site |
| `iam_rls_violations_total` | 2 | Counter | `violation_type` | RLS violations scraped from `rls_violation_log` (Layer 3, §4.2/§10) |
| `iam_auth_session_revoke_failed_total` | 2 | Counter | `reason` | Best-effort Realm-Provisioner session-revocation failures on suspend/removal/de-privilege (AUTH-8) — `reason` is `"transport"` or an HTTP status code, **no `tenant_id` label**. Grouped under `iam_auth_`, not `iam_org_membership_`, since any IAM service reducing a user's privilege via an RP session-revoke call can emit this identically |
| `iam_lifecycle_event_skipped_total` | 2 | Counter | `event_type` | Lifecycle events skipped by the recency guard (EVT-14). Domain-shared: Delegation/Tender-ACL/Group-Mapping consume overlapping RP-produced lifecycle events with the same last-writer-wins guard |
| `iam_lifecycle_event_lag_seconds` | 2 | **Histogram** | `event_type` | Seconds between `event.time` and consumer apply time — the SLO-3 drift signal (§16 A42). **A histogram, not a gauge**, and labeled by `event_type`, not `queue` |
| `iam_org_membership_unknown_event_acknowledged_total` | 3 | Counter | `topic`, `event_type` | Consumed events with no wired handler — sustained nonzero means a producer added a new type |
| `iam_org_membership_delegate_suspend_impact_total` | 3 | Counter | `checked` | P-7 suspensions where the advisory `delegate_impact` warning fired (§8.8.5, WFI-13) — advisory, not a block |
| `iam_org_membership_tenant_ownerless_escalated_total` | 3 | Counter | `reason` | Count of tenants that just entered the ownerless state on this write (TM-12) — every increment should page |
| `iam_org_membership_seat_overage_started_total` | 3 | Counter | `cause` | Transitions from under-cap to over-cap on tenant seat consumption (SEAT-5) |
| `iam_org_membership_seat_limit_reached_total` | 3 | Counter | `plan` | P-6 invite attempts blocked by the SEAT-1 cap |
| `iam_org_membership_invite_throttled_total` | 3 | Counter | `reason` (`cooldown`\|`rate_limit`) | P-6 invites refused by the abuse guards (§16 A41, PI-11/PI-12) |
| `iam_org_membership_realm_sync_failed_total` | 3 | Counter | `stage` | `realm-config-sync` reconciler failures (T-15) |
| `iam_org_membership_delegate_removal_blocked_total` | 3 | Counter | `scope` | Removals blocked by the WFI-3 delegate-impact pre-check (`409 workflow_resolution_required`, §8.8/§8.8.4) |
| `iam_org_membership_delegate_reassignment_total` | 3 | Counter | `action` (`replace_delegate`\|`stop_workflows`) | P-26 removal-resolution completions — both actions share this one counter, disambiguated by `action`, not two separate metrics |
| `iam_org_membership_membership_exists_check_total` | 3 | Counter | `caller` (`tender_acl`\|`delegation`\|`unknown`), `result` (`active`\|`inactive`) | I-15 grant-time membership-existence checks served. Only this service owns membership data, so this can't be domain-shared |
| `iam_org_membership_tenant_ownerless` | 3 | Gauge | — | Tenants with `ownerless_since IS NOT NULL` (T-13). Sustained > 0 pages |
| `iam_org_membership_realm_sync_pending` | 3 | Gauge | — | Tenants with `realm_sync_pending=true` (T-15) — **no paired `_failed_total` gauge/counter of this name**; the real failure counter is `iam_org_membership_realm_sync_failed_total` (labeled `stage`, above) |
| `iam_org_membership_seat_overage_active` | 3 | Gauge | — | Tenants with `overage_since IS NOT NULL` (SEAT-5) |
| `iam_org_membership_pending_invitations_stale` | 3 | Gauge | — | Pending invitations past `expires_at` that `invitation-expiry` hasn't flipped yet — carries no `tenant_id` label |
| `outbox_dead_letters_total` | — | Counter | `event_type` | Dead-letter events (from `platform-events`, not `iam_`/`platform_`-namespaced — a library metric, out of this standard's scope) |

**Deliberately not implemented:** `platform_queue_depth`/`platform_dlq_depth` (would need a new SQS `GetQueueAttributes` polling goroutine — a new AWS call + IAM permission, not added without an explicit decision) and `platform_retry_total` (the outbox publisher's retry-on-publish-failure loop lives inside the vendored `platform-events` library, not this repo — not ours to instrument from here).

**Not yet implemented (Phase-6 target, per `business.go`'s own header comment — do not treat as current fact):** `iam_org_membership_membership_joins_total`/`_leaves_total`, `iam_group_mapping_resolution_errors_total`, `iam_org_membership_memberships_cache_hit_ratio`, `iam_org_membership_membership_lookup_latency_seconds`, `iam_org_membership_invitations_created_total`/`_accepted_total`/`_expired_total`/`_revoked_total`, `iam_org_membership_invite_kc_cleanup_pending`/`_failed_total`. None of these currently exist in `internal/adapter/outbound/metrics/business.go`.
...
**Alerts:** `outbox_dead_letters_total rate > 0` → page; `iam_org_membership_tenant_ownerless > 0` → page `platform_operator` (§16 A39/TM-12); `platform_dlq_messages_total{reason="future_time_clamp"} rate > 0` → page (producer clock skew, EVT-15); `IAMConsumedEventSchemaViolation` — `sum by (event_type) (rate(platform_dlq_messages_total{reason="schema_violation"}[5m])) > 0` for 5m → page (producer contract break or stale local `asyncapi.yaml`, EVT-17; `deploy/monitoring/app-alerts.yml` + `deploy/helm/templates/prometheusrule.yaml`); `iam_lifecycle_event_lag_seconds > 30` sustained → page (SLO-3 drift, §16 A42); `iam_org_membership_realm_sync_pending > 0` sustained (or any un-applied `local_accounts_enabled` **disable**) → page (§16 A58/T-15); a sustained `iam_auth_session_revoke_failed_total` rate → page (fast privilege-cutoff degraded, AUTH-8); **a sustained `iam_org_membership_dependency_call_failures_total` rate for `target_service="catalog"` with `outcome="fallback_served"` → warn** (Catalog degraded; `planDefaults`/department checks are serving stale last-known-good — correctness holds, freshness doesn't); **`iam_org_membership_dependency_call_failures_total` for `target_service="group_mapping"` sustained → warn** (JIT logins delayed while Group-Mapping is down, §8.5); **`iam_org_membership_dependency_call_failures_total` for `target_service="delegation"` sustained → warn** (dept-scope removal precision degraded to tenant-wide, §8.8.4 — still correct); `iam_org_membership_dependency_call_failures_total` for `target_service="group_mapping"`, `outcome="5xx"` rate > 5/min → warn (JIT resolution failures — there is no separate `iam_group_mapping_resolution_errors_total` metric; this signal is folded into the general cross-service error counter, §11.2); a sustained rise in `iam_org_membership_delegate_removal_blocked_total` without matching resolutions → warn (UX/Workflow friction); a spike in `iam_org_membership_seat_limit_reached_total` for one tenant → informational note to the account's CSM/Billing contact. Every alert above queries the Tier-3 predecessor, never the Tier-1 `platform_dependency_*`/`platform_duplicate_messages_total` candidates directly — those remain Status: Proposed (`registry.go`) until ratified, enforced by `.github/scripts/metrics-registry-lint.sh` in CI.

**Metric naming / cardinality posture (§16 A50/A48).** This service emits metrics per the IAM Platform Observability Standard's three-tier hierarchy (`platform_*`/`iam_*`/`iam_org_membership_*` — see above), enforced by `.github/scripts/check-metric-naming.sh` in CI. A metric unique to this service's own business logic is named `iam_org_membership_<metric>`, disambiguating it from any equivalently-shaped metric another IAM service might emit without relying on a label; a metric representing a concept common across IAM-domain services (RLS violations, session revocation, lifecycle-event bookkeeping) stays generic (`iam_<metric>`) and is disambiguated by the `service` const label; a metric common across every domain (message lifecycle, cross-service dependency calls) stays fully generic (`platform_<metric>`) and adds `domain` alongside `service`. `tenant_id` labels are a bounded, deliberate choice (capped by tenant count, HLD §14.5); no metric carries an unbounded or user-supplied label. The revisit threshold (drop `tenant_id` from the highest-churn counters past ~10k tenants) is unchanged.

**Registry ratification (§16 A50, rules 11/12).** Three Tier-1 candidates (`platform_dependency_request_seconds`, `platform_duplicate_messages_total`, `platform_dependency_errors_total`) are shadow-emitted alongside a Tier-3 predecessor from the same call site — `PlatformRegistry` (`internal/adapter/outbound/metrics/registry.go`) carries each candidate's full ratification packet (semantic definition, label vocabulary, cardinality justification, aggregation expectations) at `Status: StatusProposed`. `.github/scripts/metrics-registry-lint.sh` (CI) fails if a Proposed candidate is ever referenced inside a live `expr:` in `deploy/monitoring/app-alerts.yml`, `deploy/monitoring/slo-rules.yml`, or `deploy/helm/templates/prometheusrule.yaml` — only its Tier-3 predecessor may source a rule until the entry's `Status` flips to `StatusCanonical` following real governance approval, at which point the standard's Backward Compatibility migration order applies (dashboards → alerts → recording rules → SLOs → HPA → deprecate the Tier-3 predecessor → remove after sunset). Mirrors `iam-realm-provisioner`'s own `registry.go`/lint-script pair.

### 11.3 OpenTelemetry tracing

`platform-gincommon.InitTracingFromEnv()` + `platform-pgcommon.NewOTelQueryTracer`. The retained cross-service calls create parent/child spans: I-10 creates a "jit.resolve" span with a child for the Group-Mapping `group-resolution` HTTP call and the DB write; §8.8 removal creates a "membership.remove" span with children for the Workflow `delegate-impact` call and (on the dept-scoped path) the Delegation `dept-delegate` call. W3C `traceparent` is propagated to every downstream via each of the six outbound clients' shared `httpx.NewClient(timeout)` transport (`otelhttp.NewTransport`) — **not** a per-package `propagateTraceparent` helper, and **not** `gincommon.PropagateHeaders` either (corrected rev 2.5 — see §3.1). *(The monolith's "delegation.create" span moved to the Delegation Service.)*
...
| `GLUE_REGISTRY_MEMBERSHIP_NAME` / `GLUE_REGISTRY_MEMBERSHIP_ARN` | (empty → `NoopCodec`, plain JSON on that topic) | `iam-membership-events` registry name/ARN. When set, each produced schema's version UUID is resolved once at startup via `glue:GetSchemaByDefinition` (§7.3.1) — an unregistered definition fails startup |
...
| `SUBSCRIPTION_GRACE_DAYS` | `30` | I-16's (§16 OQ-9/RP-C3) cancellation-to-suspension grace window — the threshold `ListSubscriptionLapses` filters `cancelled_at` against. O&M owns this value so RP's sweep never has to keep its own copy of the same rule |
...
| # | Invariant |
|---|-----------|
| CONFIG-1 | All infrastructure endpoints — including the six outbound client base URLs — are environment-supplied; no environment-specific value is compiled into the binary (only `BUILD_VERSION`). The same image runs everywhere. |
...
| CONFIG-5 | Glue registry names map 1:1 to the SNS-topic ownership model — one registry per topic (§7.3, SCHEMA-7); an unset name falls back to `NoopCodec` on that topic. Consumers need no registry config (`GlueDecoder` only strips the header), and DLQ routing needs no env var — each DLQ is resolved from its source queue's `RedrivePolicy` (IAM: `GetQueueAttributes` + `SendMessage` on the DLQs, already in `deploy/iam/policy.json`; `glue:GetSchemaByDefinition` on both registries likewise). |
...
**Event pipeline (rev 2.12):**

- `cmd/server/dlq_test.go` — EVT-15 poison pill (bare and wrapped) and schema violation are `SendMessage`'d to the DLQ with `EventType`/`DLQReason` attributes and acked; send failure returns the original error (redrive fallback, nil-logger safe); other outcomes pass through untouched; `RedrivePolicy` → DLQ URL resolution (real-AWS and floci endpoints, error cases); unresolvable DLQ → unwrapped handler; `buildSQSConsumer` decodes a Glue-encoded upstream payload.
- `cmd/server/inbound_schema_test.go` — valid / open-schema-extra-fields payloads reach `Handle`; violations are rejected before it; unknown type passes through for `ackUnknown`; handler errors propagate unchanged; end-to-end `inboundHandler` sends a violation to the DLQ; `TenantRealmReady` matches Realm Provisioner's producer shape.
- `eventbus/validating_codec_test.go` — `Validate` wraps `ErrNoSchema` for unknown types and enforces consumed schemas.
- `eventbus/glue_codec_test.go` — startup sends the registered definition; non-`AVAILABLE` version and missing embedded file fail startup; `GlueDecoder` strips the header and its `Encode` always fails; `TestRegisteredDefinition_MatchesSchemaGov` pins `registeredDefinition` byte-for-byte against real Python `json.dumps(…, separators=(",", ":"))` over all 28 embedded schemas (plus an `asciiEscape` ↔ `ensure_ascii` parity test).
...
- **Glue version resolution by definition (rev 2.12, `test/integration/glue_codec_test.go`, floci Glue):** register v1, then a BACKWARD-compatible v2 of the same schema → a codec built from the v1 embedded file still stamps v1's UUID (never latest); every produced schema resolves; an unregistered definition fails startup.
...
| OQ-6 | **Event schema shape: CLOSED, then reversed to OPEN (rev 2.11) after a real platform-policy conflict surfaced.** Originally: this LLD's rev ~2.3–era text and `api/asyncapi.yaml`'s `info.description`/`x-forward-compatibility` blocks documented a **deliberate** CLOSED-schema contract (`additionalProperties: false` on all 13 then-produced payloads) — no free forward-compatibility, every field addition a coordinated producer+consumer rollout and Glue version bump. That text was internally consistent and correctly reflected the code at the time (first resolved as such, historically, before this entry was rewritten). **Reversed 2026-09-21 (rev 2.11)**: running the real pinned `platform-schemagov:0.4` image directly proved its Pass 5 (open-schema guard) hard-blocks *any* closed schema with no per-service exception mechanism — a genuine, unresolvable conflict between this service's documented architecture and current platform tooling policy, not a bug fixable by editing files alone. Decision: adopt the OPEN-schema model instead (all 14 produced payloads now `additionalProperties: true`, matching `iam-user-profile`), since the platform's own governance tool leaves no room for a documented exception. `asyncapi.yaml`'s `info.description`/`x-forward-compatibility` blocks and this LLD's SCHEMA-5 (§7.3.1) are rewritten accordingly. Consumers of this service's 14 produced events must now tolerate unknown fields (lenient deserialization, per `asyncapi.yaml`'s `required-settings`) — this is the real, one-time rollout cost of the reversal. | IAM platform | Resolved |
...
| OQ-9 | **RP-C3 (subscription-lapse sweep) blocker — Resolved.** RP's own lapse sweep (RP-C3, T-16/OQ-7) emits `TenantSuspended(source=billing_lapse)` once a `cancelled` tenant's grace period elapses, but RP reported it had no way to learn `cancelled_at` at all — O&M's only production onto `iam.tenant.events` (the one topic RP partially consumes, via `tenant-realm-q`) is `TenantCreated`/`TrialStarted`; nothing carries cancellation data. **New: I-16** `GET /api/v1/internal/subscription-lapses` (§5.4) — RP polls this instead of tracking `cancelled_at` itself. O&M computes the grace-period math (`SUBSCRIPTION_GRACE_DAYS`, default 30, §12) so the rule stays single-sourced instead of drifting between two independently-configured services; the response carries `{tenant_id, realm_id, realm_type, cancelled_at}`, enough for RP to act with no second lookup. **Deliberately pull, not push, unlike RP-4's resolution (§18.3) for the analogous trial-expiry case** — `trial_ends_at` is one-directional (a trial only ever expires, never un-expires) so RP can safely cache it once via `TrialStarted` and sweep locally forever after; a subscription cancellation is reversible (`TenantReactivated` before grace elapses), which would force a push model to also model "un-cancel" as a second event RP must consume and reconcile against its own local state, with the attendant missed-delivery/ordering risk. Pull avoids that: O&M's `tenants` row is already the single source of truth, so RP simply asks it fresh on every sweep. Self-idempotent — once RP acts (emitting `TenantSuspended`), the tenant's `status` flips to `suspended` and it naturally drops off the next poll, no ack/cursor needed. **Header correction (found + fixed):** despite being cross-tenant, RP's poll still sends a sentinel `x-tenant-id: 00000000-0000-0000-0000-000000000000` (uuid.Nil) — an initial version of the client sent no `x-tenant-id` header at all, which is correct in isolation but is unconditionally rejected (`401`) by `platform-gincommon`'s shared `RequireAuth` middleware on every `/api/v1` route in every IAM service, with no per-route opt-out. O&M's handler never reads the header's value (it queries the BYPASSRLS `sysPool` regardless of tenant context); the sentinel exists purely to satisfy that platform-wide contract. | IAM platform + RP | Resolved |
...
| 403 | `service_account_not_grantable` | Membership-create (P-6/I-3) or role-grant (P-10/P-28) target resolves to a `service_account`-typed Keycloak principal (the per-tenant `platform-automation` automation principal, HLD §5.8) — non-members cannot be enrolled or granted a role (AUTH-9). Defense-in-depth ahead of the composite-FK bar (TR-8/DM-4); no state change. |
...
| 503 | `dependency_unavailable` | Generic — declared and status-mapped (503), but **not currently raised by any path** (corrected, rev 2.3): `wrapConnErr` used to default every *unrecognized* low-level error to this code, silently masking a caller's own business error under a misleading 503 — a bug found and fixed in `iam-realm-provisioner`'s identical helper, then found here too and fixed the same way; an unrecognized error now passes through unchanged instead. Reserved for a future dependency needing its own code distinct from `db_unavailable`. |
| 503 | `db_unavailable` | Postgres `/readyz` health-check failure. **Also** what `TxRunner.RunInTx`/`withPool`'s `wrapConnErr` (`db.go`) maps a low-level connection/resource error into at write/read time (corrected, rev 2.3 — previously described as `dependency_unavailable`): SQLSTATE class 08 (connection exception) and 53 (insufficient resources) via `platform-pgcommon` v1.3.0's `IsConnectionException`/`IsInsufficientResources`; class 57 (operator intervention) and 58 (system error) via a hand-rolled match on the `"SQLSTATE 57…"`/`"SQLSTATE 58…"` text pgconn puts in `Error()`, since pgcommon has no dedicated helper for those two; and `puddle.ErrClosedPool` on a closed pool. A `*domain.DomainError` already produced by the wrapped callback (a business error, not a connectivity failure) passes through unchanged — `wrapConnErr` never masks it. `HandleError` (§middleware) independently re-classifies a leaked raw `*pgconn.PgError` of these same classes into this same 503, as a fallback for a future missed `wrapConnErr` call site. |
...
| `GET /internal/subscription-lapses` (I-16) | Realm Provisioner (RP-C3) → This | §18.3 | periodic sweep, not latency-sensitive | Core down means RP's sweep sees a stale/empty list this cycle — no lapsed tenant gets suspended late by more than one sweep interval; no incorrect suspend | §16 OQ-9 |
...
Every user-removal, dept-demotion/removal, and resolution call blocks synchronously on `WorkflowClient` (WFI-7). A Workflow outage doesn't corrupt state (`503 workflow_service_unavailable`, no DB write, WFI-8) but stops admin-initiated removals/demotions from completing until it recovers — no cached fallback, no defer-and-retry; the caller retries. Watch `iam_org_membership_delegate_removal_blocked_total` (normal blocks) versus a spike in `503 workflow_service_unavailable` (a Workflow dependency issue, not an O&M fault).

### 20.4 Seat-limit signal, not an incident (§16 A10)

`409 seat_limit_reached` and a sustained `iam_org_membership_seat_limit_reached_total` rate are expected product behaviour, routed to the account's CSM/Billing contact, not on-call. The one page-worthy pattern: `seat_limit_reached` for a tenant whose `licensed_seats` should have just increased via a recent `TenantSeatsChanged` — check `billing-orgm-q` consumer lag and `processed_events` for the expected event ID.
...
**This service — consumer side (done and tested, but still inert):**
...
- **Corrected 2026-09-20**: now has real test coverage (`catalog_consumer_test.go`, 5 tests covering dedup/unknown-type/invalid-id/cache-error paths) — the earlier "no test coverage" claim is stale. Still inert until `SQS_CATALOG_ORGM_QUEUE_URL` is set and something upstream actually publishes `DepartmentCatalogChanged` — neither is true today
- **Also added 2026-09-20**: the `catalog-orgm-q`/`DepartmentCatalogChanged` channel was missing from `api/asyncapi.yaml` entirely despite the consumer code existing — added (`sqsCatalogOrgm` server, `catalogOrgmQueue` channel, `consumeCatalogOrgm` operation, `DepartmentCatalogChanged` message + envelope/payload schemas, plus the matching `internal/adapter/outbound/eventbus/schemas/department_catalog_changed.json`). The payload schema is deliberately a minimal open placeholder (`additionalProperties: true`, no fields) — `catalog_consumer.go` doesn't decode any field, and iam-catalog-admin has not defined a real contract (see below), so inventing field names would be fabrication, not documentation. Verified against a real `schema-gov` validation run (locally-cached image, not the CI-pinned one — see the two caveats after the status table below) — passes every applicable pass.

**Catalog Admin — publisher side (corrected 2026-09-17: nothing exists yet):**

This section previously claimed the publisher side's architecture was done
(`port.EventNotifier`, `NoopEventNotifier`, `notify()` call sites, Terraform
IaC). Verified against `iam-catalog-admin`'s actual source: none of it
exists — zero matches for `EventNotifier`/`DepartmentCatalogChanged`/
`notify()` anywhere in that repo, and no `deploy/messaging/` Terraform in
either repo. See `gap_doc/pending_gap_sep15.md` Gap 12 for the corrected
status table and the full remaining scope (infra Terraform + `port.EventNotifier`
+ `NoopEventNotifier` + `DepartmentService` call sites + `main.go` wiring +
`SNSEventNotifier`, all still to be built in `iam-catalog-admin`).

**Remaining — full publisher side, infra, and this service's test coverage:**

| What | Who | Status |
|------|-----|--------|
| `port.EventNotifier` interface + `NoopEventNotifier` in catalog-admin | Catalog developer | ⏳ Not started |
| `DepartmentService` calls `notify()` after every write | Catalog developer | ⏳ Not started |
| `main.go` wired with `NoopEventNotifier` | Catalog developer | ⏳ Not started |
| Apply `deploy/messaging/sns_infrastructure.tf.example` | Infra team | ⏳ Not started (file does not exist yet) |
...
| Test coverage for `catalog_consumer.go` | This service's developer | ✅ Done 2026-09-20 (`catalog_consumer_test.go`) |

**Two schema-governance caveats found while adding the AsyncAPI declaration (2026-09-20), confirmed real by a real CI run against the pinned `platform-schemagov:0.4` image (2026-09-21 rev 2.8), one fixed cleanly, one over-corrected then fixed properly (rev 2.9):**
1. Pass 5's "Open-schema guard" rejects `additionalProperties: false` on any event schema — **but only 14 of the 28 events (the *consumed*, other-service-owned ones) are supposed to be open**; the other 14 (this service's own *produced* events) were at this point deliberately CLOSED per §16 OQ-6 ("Resolved") and `asyncapi.yaml`'s own `x-forward-compatibility` block, which explicitly forbade flipping them without an org-wide decision. Rev 2.8 missed this distinction and flipped all 28 to `true`. Rev 2.9 reverted the 14 produced schemas back to `false` and, at the actual root cause, added an explicit `additionalProperties: true` to the 14 consumed schemas' `asyncapi.yaml` components (previously left implicitly unset, which `schema-gov extract` was defaulting to `false` on regeneration). **Superseded 2026-09-21 (rev 2.11)**: the org-wide decision this section says Pass 5 was blocked on was made — the CLOSED-schema architecture is reversed to OPEN, all 14 produced schemas now correctly declare `additionalProperties: true`, and Pass 5 passes clean for the first time. See rev 2.11's changelog entry and OQ-6 for the current, final state; this note is kept for the historical record of how the conflict was discovered.
2. `api/asyncapi.yaml` references `mfa_reset.json` (via schema-gov's own snake_case derivation from `MFAReset`/`MFAResetPayload`, not a literal filename string anywhere in the spec), but the committed schema file was named `mfareset.json` (no underscore) — traced to a real prior commit (`0f74f29 fix: sync event schemas with schema-gov extract 0.4`). Renamed the file to `mfa_reset.json`; updated `glue_codec.go`'s `schemaFileNames` map (production event-type lookup) and every other reference (`scripts/init-floci.sh`, `.github/scripts/stage-produced-event-schemas.sh`, `glue_codec_fs_test.go`, `glue_codec_test.go`, `test/unit/domain_test.go`) to match. This part of rev 2.8 was correct and needed no follow-up.
...
| 12 | DepartmentCatalogChanged event | ⏳ This service's consumer done, tested, and now declared in `api/asyncapi.yaml` (2026-09-20); catalog-admin publisher side + infra Terraform not started — chain still fully inert |
| 13 | Tracing broken | ✅ Verified working (no code change) |
| 14 | SNS subscription groupmap-q | ✅ Terraform IaC added in Group Mapping |
| 15 | Glue header decode | ✅ Fixed in Group Mapping; this service's own consumers likewise strip the header now (`eventbus.GlueDecoder` via `WithConsumerCodec`, rev 2.12) |

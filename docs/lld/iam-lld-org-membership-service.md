...
│           ├── httpx/                       # otelhttp-instrumented shared http.Client factory — SCAFFOLDED, NOT adopted: zero production callers, all five outbound clients above still hand-build their own http.Client + local propagateTraceparent (§3.1)
...
The five outbound clients (`workflow`, `realmprovisioner`, `catalogadmin`, `groupmappingclient`, `delegationcheck`) add **no** new third-party dependency — each is a plain `net/http` client behind a `core/port` interface, wired at `main.go`, mesh-only over mTLS, built on the identical construction pattern: a hand-built `http.Client` plus each package's own local `propagateTraceparent(ctx, req)` helper (`otel`/`propagation.NewCompositeTextMapPropagator`, **not** `gincommon.PropagateHeaders` — corrected, rev 2.3; confirmed by grep against all five `http_client.go` files), and a configurable `*_TIMEOUT_MS` env var. `internal/adapter/outbound/httpx` (`httpx.NewClient`, an otelhttp-instrumented shared `http.Client` factory) exists and is 100%-covered by its own tests, but has **zero production callers** — it was evidently scaffolded to replace this hand-rolled construction pattern across all five clients, but that migration was never finished; nothing in this document should be read as claiming otherwise. There is no `userprofile` client any more — deleted as dead code once delegation's OOO coordination moved out (LLD §16 OQ-5).
...
**HTTP adapter** (`adapter/outbound/workflow/http_client.go`), same construction pattern as the other four outbound clients (`realmprovisioner`, `catalogadmin`, `groupmappingclient`, `delegationcheck`) — a local `propagateTraceparent` helper for W3C trace propagation (**not** `gincommon.PropagateHeaders`, corrected rev 2.3 — see §3.1), configurable timeout (`WORKFLOW_TIMEOUT_MS`, §12):
...
**This service — consumer side (done, but unverified and inert):**
...
- Has no test coverage, and is inert until `SQS_CATALOG_ORGM_QUEUE_URL` is set and something upstream actually publishes `DepartmentCatalogChanged` — neither is true today
...
| Test coverage for `catalog_consumer.go` | This service's developer | ⏳ Not started |
...
| 12 | DepartmentCatalogChanged event | ⏳ This service's consumer done (untested, inert); catalog-admin publisher side + infra Terraform not started |

...
| **Group→role/department mappings** | Three tables: `group_dept_role_mappings`, `group_tenant_role_mappings` (§16 A25), `group_dept_mappings` | Keycloak group membership — Keycloak |
...
**Does NOT own:** credentials / MFA enforcement / JWT issuance (Keycloak); display identity / signature / OOO presentation flag (User Profile); realm or Keycloak Admin API mutations (Realm Provisioner — this service **never** calls the Keycloak Admin API); workflow template authoring or `assignee_overrides` state (Workflow Service); audit records (Audit Log); tender content / bids (Tender Service); metered quota consumption (Usage & Metering); pricing / currency / seat enforcement past grace (Billing).
...
| **`member` role never persisted** | `chk_tr_no_member` bars `member` on `tenant_roles`; `chk_gtrm_no_member` bars it on `group_tenant_role_mappings`. The role is injected at read time by I-8 at the projection layer (TR-7 / §16 A29) |
